...
> 대상 코드: [`ai/voice/`](../) · 갱신: 2026-07-30 · 기준: `origin/develop`
> 관련 Jira: 49, 50, 69, 109~120, 157, 159, 165
> 설계 기준 명세: 33장 피해자 음성 상호작용([`docs/07-AI-탐지-음성.md`](../../../docs/07-AI-탐지-음성.md))
>
> **이 문서가 `ai/voice` 문서의 단일 기준이다.** 원본 측정 표는 [`data/`](measurements/)에만 둔다.
> 코드 사용법과 폴더 구조는 [`ai/voice/README.md`](../README.md)를 본다.
...
> Jira: S15P11A301-114 · 코드: `sentinel_voice/guide_audio.py` · 파일: `ai/voice/assets/`
...
| **시간 확장** | **`atempo=0.8`** (S15P11A301-260) |
| 변환 규격 | `atempo` → `loudnorm=I=-20:TP=-2:LRA=7` → 16kHz mono PCM 16-bit |
| 생성일 | 2026-08-03 (문구 v2) · 2026-08-04 재변환 (시간 확장) |

#### 시간 확장을 하는 이유 (S15P11A301-260, 2026-08-04)

**원본이 너무 빨랐다.** 안내가 빠르다는 의견이 나와 재봤더니 사실이었다.

| 문구 | 확장 전 | 확장 후 (`atempo=0.8`) |
|---|---|---|
| 주변에 다른 인원이 있습니까? | 1.46초 · **8.2 음절/초** | 1.80초 · **6.7 음절/초** |
| 제 말이 들린다면 대답해주십시오. | 1.74초 · 8.0 | 2.14초 · 6.5 |
| 움직일 수 있습니까? | 1.03초 · 7.7 | 1.26초 · 6.3 |
| 다친 곳이 있으십니까? | 1.28초 · 7.0 | 1.58초 · 5.7 |
| 구조 로봇입니다. 들리면 대답해 주세요. | 2.27초 · 7.0 | 2.82초 · 5.7 |
| 구조 요청이 전달되었습니다… | 3.17초 · 6.9 | 3.94초 · 5.6 |
| **총 재생** | **10.96초** | **13.55초** |

일상 대화가 5~7 음절/초다. **안내 방송인데 대화보다 빨랐다.** 원인은 생성 설정이다 —
Speed를 `1.1`로 줬다(위 표). 일부러 10% 빠르게 만든 값이다.

**MiniMax에서 다시 뽑지 않는다.** Speed를 내려 재생성하면 **톤이 함께 달라진다**(팀
확인). 현재 톤은 유지하기로 했으므로 생성 설정은 그대로 두고 변환에서 시간만 늘린다.

**`atempo`를 쓰는 이유** — 음높이를 바꾸지 않고 시간만 늘린다(구간 중첩 가산).
리샘플로 늘리면 음높이가 함께 내려가 톤이 변하며, 그것은 이 작업의 목적과 정반대다.

**순서가 중요하다** — `atempo` **다음** `loudnorm`이다. 반대로 하면 정규화 이후
시간축이 바뀌어 측정 근거와 결과가 어긋난다.

**원본에서 다시 변환한다.** 이미 만든 16kHz 자산을 재처리하지 않는다 — 손실을 겹치지
않기 위해서다. `TEMPO = 0.8`은 **원본이 Speed 1.1일 때** 맞는 값이다. 원본을 다른
Speed로 다시 뽑으면 이 값도 다시 골라야 한다.

**영향** — 총 안내 재생시간이 2.6초 늘어난다. 세션 제한 180초와 실측 E2E 75~111초에
영향이 없다. 최대 파일 길이 3.94초는 규격 상한 15초에 여유가 있고, 6개 전부
`validate_wav`를 통과한다. 문구는 한 글자도 바꾸지 않았으므로 에코 가드·재생 승인
대조에도 영향이 없다.

> **음높이는 청취로 판정했다.** 자기상관 기반 f0 측정에서 1초대 짧은 문구 두 개가
> ±5~8%로 나왔으나, `atempo`는 원리상 음높이를 바꾸지 않으며 짧은 구간에서 그 측정이
> 흔들린 것으로 본다. 나머지 네 개는 ±2% 안이었다. **최종 확인은 귀로 했다** —
> §6-2의 감쇠 상한과 같은 원칙이다(사람이 소비자인 값은 지표가 아니라 귀가 기준).
...
MiniMax 원본은 `ai/voice/`에 `mini_*.wav`로 저장하며 **커밋 대상이 아니다.** 일괄 변환:
...
```bash
cd ~/projects/S15P11A301/ai/voice && cp -n .env.example .env
# GMS_KEY=<팀 GMS 키> 입력
```
...
```bash
python -m evaluation.gms_model_bench --dry-run                    # 호출 없음
python -m evaluation.gms_model_bench --models gpt-5.4-mini --runs 1 \
  --case-id all-fields-urgent --confirm-live                 # 스모크
python -m evaluation.gms_model_bench --models <목록> --runs 2 --confirm-live
```
...
| 항목 | 왜 남았나 |
|---|---|
| ~~C 무응답 경로 (`anyResponseDetected=false`)~~ | **실기 시험을 생략하기로 결정**(2026-08-04, 165). 무음 판정은 URGENT 턴 실기 작동으로 확인됐고 나머지는 결정적 경로다. 근거와 남는 위험은 §11-3 |
| ~~STT WER (공개 코퍼스)~~ | **측정됨** — KsponSpeech `eval_clean` 2,471발화. clean CER 0.141 → SNR 0dB 0.366. `measurements/STT-오류율-실측.md` |
| STT WER · 핵심 표현 보존율 (**우리 도메인**) | 위 값은 일상 대화다. 우리 마이크·어휘·약한 발화는 빠져 있어 실전 정확도가 아니다. 정답 확정 녹음이 **화자 1명 16발화**뿐이고 §10-1의 최소량은 3명×10발화×4조건=120개다. **120은 이 칸을 비운 채 CONDITIONAL로 닫았다**(§10-7) |
| **STT RTF · E2E 4구간 지연** | 젯슨 기준이어야 하며 벤치에 단계별 계측이 없다(§10-6). §10-7의 조건 2 |
| GMS 왕복 지연 P50/P95 · 일관성 | bench 재실행. 단 §10-7에 기록된 값(P95 2.12초 · 완전일치 94.74%)이 있다 |
...
**도구** — [`evaluation/jetson_resource_bench.py`](../evaluation/jetson_resource_bench.py)가 세 구간을
자동으로 나눈다.
...
python -m evaluation.jetson_resource_bench \
  ...
  -- bash -lc 'python -u -m evaluation.pipeline_bench 2>&1 | tee results/jetson-resource/voice-gms/console.log'
```
...
### 9-8. 입력 장치 무음 사건 (2026-08-04, S15P11A301-257)

**발견 경로.** EC2 담당이 리허설 이벤트 영상의 원본 오디오를 측정했더니 **295초 전체가
디지털 무음(peak 0)** 이었다. 시스템은 아무 오류도 내지 않았다 — 잡음 제거는 성공했고
자산은 `AVAILABLE`이 됐다. **사람이 손으로 재봐서 알게 됐다.**

**원인.** 젯슨에서 확인한 값이다.

| 확인 | 값 |
|---|---|
| `pactl info` 기본 소스 | `alsa_input.platform-sound.analog-stereo` — **보드의 아날로그 입력 단자** |
| 그 단자 상태 | 아무것도 연결돼 있지 않다 |
| BRIO 100 소스 | 존재 · `Mute: no` · **`SUSPENDED`(아무도 읽지 않음)** |
| `arecord -l` | `card 0: B100 [Brio 100]` — 하드웨어 정상 |
| 조치 후 `arecord` peak | **0 → 2913** |

**하드웨어 고장도 의도적 음소거도 아니었다.** `Mute`는 전부 `no`였다. 기본 입력 소스가
빈 단자로 지정돼 있었을 뿐이다. 빈 단자는 정확히 0을 낸다.

**왜 STT까지 영향이었나.** `record_microphone`은 `sd.rec()`에 장치를 주지 않아 PortAudio
기본 장치를 쓴다. 젯슨에서 그것은 index 26 `default`이고, `default`는 PulseAudio를 거쳐
**PulseAudio 기본 소스**로 간다. 녹화 파이프라인의 `pulsesrc`도 같은 기본값을 쓴다.
**한 값이 어긋나면 두 경로가 같이 어긋난다.** 실측으로 확인했다.

```text
기본 입력 index = 26 → default
peak = 0.0701  rms = 0.0158     ← 소스를 BRIO로 바꾼 뒤
```

**젯슨에서는 마이크를 이름으로 지목할 수 없다.** PulseAudio가 USB 카드를 독점하면 ALSA
직접 접근이 막혀, PortAudio 목록에 그 카드가 **아예 나타나지 않는다.** `arecord -l`은
`card 0`을 보여주는데 PortAudio 목록은 `hw:1`(HDMI)부터 시작했다. 그래서
`tools.check_audio_io`의 기존 기본값 `--input-match "BRIO"`는 **젯슨에서 항상 실패**했다.
실효 수단은 `pactl set-default-source` 또는 `PULSE_SOURCE`다.

**무엇이 더 무거웠나.** 무음 자체보다 **무음을 무응답으로 보고하던 것**이 무거웠다.

```text
무음 → raw_rms < SILENCE_RMS → NO_VOICE_DETECTED
     → anyResponseDetected=false → riskLevel=IMMEDIATE
```

마이크 사망이 **"의식 없는 요구조자"로 관제에 보고된다.** §10-3 치명 오류 목록의
"시스템 장애를 요구조자 무응답으로 변환"에 정확히 해당한다. 목록에 적어 두고 코드에서
막지 않았다.
...
| 어디 | 무엇 |
|---|---|
| `session_runner.listen` | 무음 판정 **이전에** 디지털 무음을 갈라 `audio_error`로 보낸다 → 기존 `AUDIO_DEVICE_ERROR` → `FAILED_AUDIO` 경로 재사용. `safety.SYSTEM_FAILURE_TERMINATIONS`가 이미 무응답 근거로 쓰지 않는다 |
| `config.SILENT_INPUT_PEAK` | `1e-6`. `SILENCE_RMS`(0.005)와 세 자리 이상 떨어져 있어 진짜 무응답을 삼키지 않는다 |
| `pipeline.record_microphone` | `SENTINEL_INPUT_DEVICE`로 장치를 지정할 수 있게 했다. 미지정 시 현행 동작 |
| `tools.check_audio_io` | 리눅스 기본값을 `pulse`로. 녹음 결과가 디지털 무음이면 **실패로 끝낸다** |
| `ai/voice/denoise` | `SilentAudioTrack`을 `NoAudioTrack`과 갈랐다. 무음은 업로드하지 않고 `[ALERT]`를 `scan.log`에 남긴다 |

**조용한 방과 죽은 경로의 구분이 이 조치의 핵심이다.** 살아 있는 마이크는 조용해도 정확히
0을 내지 않는다 — 우리 실측에서 조용하다고 판정된 구간도 `rms 0.0038`이었다. 전 구간이
정확히 0인 것은 캡처 경로 사망에만 나타나는 서명이다. 단위 테스트가 양쪽을 함께 고정한다
(`test_dead_input_is_device_error_not_no_response`,
`test_quiet_room_is_still_no_response`).

> **테스트 픽스처도 함께 고쳤다.** 기존 테스트는 "무음"을 `np.zeros`로 모사했다. 그것은
> 물리적으로 불가능한 신호이며, 실제로는 존재하지 않는 상황을 무응답 경로의 근거로 삼고
> 있었다. 조용한 방 픽스처를 실측값 `0.0038`로 바꿨다.

**남은 것 (범위 밖)** — `sentinel_streaming`의 `media.yaml` `audio_source`에 `device=`
명시는 스트리밍 담당 몫이다. 젯슨의 기본 소스를 부팅마다 고정하는 것은 운영 절차로 다룬다.
...
측정값과 판정은 **§10-7**에 있다. 아래 `기준` 열은 측정 전에 고정한 값이며 바꾸지 않았다.

| 구분 | 항목 | 기준 | 측정값 | 판정 |
|---|---|---|---|---|
| STT | clean / SNR 10 / 5dB WER | ≤ 15% / 20% / 25% | 30.4 / 36.1 / 41.3% | ⚠️ 재료 상이 |
| STT | SNR 0dB WER | 기록, 목표 ≤ 40% | 49.5% | 기록됨 |
| STT | 핵심 표현 보존율 | 전체 ≥ 90%, 숫자·가능/불가 ≥ 95% | — | **미측정** |
| VAD | 발화 검출 재현율 | ≥ 95% | — | **미측정** |
| VAD | 무음 오검출 / 비발화 오검출률 | 10개 중 0건 / ≤ 5% | — / **30.2%** | ❌ **미달** |
| STT | RTF | P95 ≤ 1.0 | — | **미측정** |
| GMS | JSON 파싱·스키마 준수율 | ≥ 99% | 100% (38/38) | ✅ |
| GMS | 핵심 슬롯 정확도 / 숫자·가능불가 | ≥ 90% / ≥ 95% | 합성 100% · **실발화 56%** | ⚠️ 표본별 상반 |
| GMS | 발화에 없는 슬롯 생성률 | ≤ 2% | 0건 | ✅ |
| GMS | 위험도 규칙 정답 일치율 | ≥ 95% | 위험→안전 오판 0건 | ✅ 부분 |
| GMS | 반복 일관성 (완전일치) | ≥ 95% | 94.74% (36/38) | ⚠️ 근소 미달 |
| GMS | 호출 성공 시 지연 | P95 ≤ 3초 | 2.12초 | ✅ |
| E2E | 트리거→최초 안내 시작 | P95 ≤ 1초 | — | **미측정** |
| E2E | STT 완료→GMS 완료 | P95 ≤ 3초 | — | **미측정** |
| E2E | 발화 종료→관제 전송 완료 | P95 ≤ 5초 | — | **미측정** |
| E2E | 전송 완료→안내 시작 | P95 ≤ 0.5초 | — | **미측정** |
| E2E | (참고) 세션 전체 < 180초 | — | 75.2~111.2초 | ✅ 참고 |
| 자원 | OOM·강제 종료 / 열 스로틀링 | 0건 / 0건 | 0건 / 0건 | ✅ |
| 자원 | 피크 시 available RAM | ≥ 1GB | 3,518MB | ✅ |
| 자원 | 세션 종료 60초 후 swap 증가 | ≤ 256MB | — | **미측정** |
| 자원 | 음성 추가 후 비전 FPS 저하 | ≤ 10% | — | **미측정** |
| 자원 | 30회 세션 후 메모리 증가 | ≤ 10% | — | **미측정** |
...
- 응답 가능 인원 수를 임의 생성
- **시스템 장애를 요구조자 무응답으로 변환** — 코드 가드 있음(§9-8, S15P11A301-257).
  입력이 디지털 무음이면 무응답이 아니라 `AUDIO_DEVICE_ERROR`로 종료한다. **2026-08-04
  이전에는 이 항목이 목록에만 있고 코드에서 막히지 않았다.**
...
[`evaluation/pipeline_bench.py`](../evaluation/pipeline_bench.py)는 STT 단일 실행 시간, GMS
...
> **이 보완은 아직 안 됐다.** 그래서 §10-7의 판정이 PASS가 아니라 CONDITIONAL이고,
> RTF·E2E 4구간이 미측정으로 남았다. 이 절의 목록이 곧 §10-7 조건 2의 작업 범위다.

### 10-7. 최종 판정 — **CONDITIONAL** (2026-08-04, S15P11A301-120)

> **CONDITIONAL** = 잰 항목은 통과했고, **못 잰 항목이 남아 있어 그 조건 아래서만
> 합격**이다. "다 했다"가 아니다. §10-2 표의 23개 항목 중 **9개가 미측정**이며,
> 완료 기준의 「빈칸 없음」은 지키지 못했다. 그 사실을 감추지 않고 아래에 적는다.

#### 무엇이 통과했나

**가장 무거운 기준이 통과했다** — `위험 → 안전 오판 0건`. GMS 모델 비교 전 단계에서
누적 143회 이상 호출하며 한 건도 나오지 않았다. 등급을 실제보다 **낮게** 보고하는
오류가 없다는 뜻이고, §10-3 치명 오류 목록의 첫 항목이다.

| 항목 | 값 |
|---|---|
| JSON 파싱·스키마 준수 | 100% (38/38) |
| 발화에 없는 슬롯 생성 | 0건 |
| GMS 지연 P95 | 2.12초 (기준 3초) |
| 젯슨 피크 시 여유 RAM | 3,518MB (기준 1GB) |
| OOM·열 스로틀링 | 0건 / 0건 |
| 세션 E2E (참고) | 75.2~111.2초 (제한 180초) |

#### 무엇이 미달했나 — 셋 다 안전한 방향이다

**① 비발화 오검출률 30.2% (기준 5%) — 명확한 미달.** 사람 말이 없는 클립 63개 중
19개가 무음 게이트와 VAD를 통과해 STT에 도달했다(`measurements/STT-오류율-실측.md`
§3-2). 다만 프라이밍 프롬프트를 제거한 뒤로는 통과해도 나오는 출력이 무해하다 —
`뭐했어요, 지금?` 같은 문장이 되어 추출이 전부 `UNKNOWN`이다. 프롬프트가 있던 시절에는
같은 소음이 `도와주세요, 다쳤어요, 가스, 화재`로 나와 **긴급 YES → IMMEDIATE**가 됐다.
즉 미달은 남았지만 **결과의 위험은 251에서 제거됐다.** 게이트를 조이지 않은 이유도
안전 쪽이다 — 요구조자의 힘없는 발화를 함께 막을 위험을 확인하지 않았다(§11-5).

**② 공개 코퍼스 WER 30.4% (기준 15%) — 재료가 다르다.** 이 기준은 §10-1이 정의한
**우리 도메인 발화**(짧은 고정 응답)를 전제로 세웠다. 실제로 잰 것은 KsponSpeech
`eval_clean` 2,471발화이며 **두 사람이 자유롭게 대화한 구어체**다. 같은 척도로 비교할
수 없다. 그래서 이 칸은 「미달」이 아니라 **「다른 재료로 잰 참고값」**으로 기록하고,
도메인 값은 미측정으로 남긴다. 판정을 유리하게 돌리려는 것이 아니라 **애초에 다른 것을
쟀기 때문이며**, 도메인 코퍼스 확보가 이 티켓의 남은 일이다.

> 함께 기록한다 — CER은 clean **0.141**, SNR 0dB **0.366**이다. **CER과 WER, 그리고
> 슬롯 정확도는 서로 다른 것을 잰다.** `피가` → `비가`는 CER 0.167(거의 맞음)인데
> 긴급 보고가 사라진다. 하나만 인용하면 과장이다(§13 용어집).

**③ 반복 일관성 94.74% (기준 95%) — 38문항 중 1건 차이.** 표본이 38개이므로 한 건이
2.6%다. 소표본에서 기준선을 스치는 값이며, 같은 시험에서 위험→안전 오판은 0건이었다.

#### 무엇을 못 쟀나 — 9개 항목과 그 이유

| 미측정 항목 | 왜 |
|---|---|
| 도메인 STT WER · 핵심 표현 보존율 | **화자 3명 이상 × 10발화 × 4조건 = 120개**가 §10-1의 최소량이다. 현재 정답 확정 녹음은 **화자 1명 16발화**뿐이다(`tools/record_corpus.py`로 수집) |
| VAD 발화 검출 재현율 | 위와 같은 코퍼스가 필요하다. 소표본 관측(12/12)은 기준 판정용 표본이 아니다 |
| STT RTF P95 | 젯슨 CPU 기준이어야 한다. PC(GPU float16) 값으로 대체 불가(`메모리-예산.md` 부록 A) |
| E2E 4구간 지연 | 단계별 monotonic timestamp 계측이 벤치에 없다(§10-6) |
| swap 증가 · 30회 세션 누적 · 비전 FPS 저하 | **통합 구동(비전+SLAM 동시)** 미실시. 팀원과 함께 해야 하며 §9-5 안전 게이트 순서를 따른다 |

#### 판정 근거

미달 3건이 **모두 과대보고(안전한 방향)이거나 재료 불일치**이고, 미측정 9건은
**검증이 안 됐을 뿐 위험이 확인된 것이 아니다.** 반대로 등급을 낮추는 오류
(위험→안전 오판)와 없는 사실을 만드는 오류(환각 슬롯)는 0건이다. 그래서 FAIL이 아니다.

동시에 도메인 정확도와 RTF·E2E·통합 자원이 비어 있으므로 PASS도 아니다.
**결론: CONDITIONAL.** 조건은 아래 세 가지다.

1. 도메인 코퍼스(화자 3명 × 10발화 × 4조건)를 확보해 STT·VAD 항목을 다시 잰다
2. 젯슨에서 RTF와 E2E 4구간을 잰다 — 벤치에 단계별 계측을 먼저 넣어야 한다(§10-6)
3. 통합 구동으로 자원 3항목을 잰다 — §9-5 안전 게이트 순서를 지킨다

#### 완료 기준 대조 — 지킨 것과 못 지킨 것

| 완료 기준 | 결과 |
|---|---|
| §10-2 전 항목에 측정값 기록 (**빈칸 없음**) | ❌ **못 지켰다.** 9개 미측정 |
| 최종 판정을 PASS/CONDITIONAL/FAIL 중 하나로 명시 + 근거 | ✅ CONDITIONAL (위) |
| 기준 미달 항목을 삭제하지 않고 원본 로그와 남김 | ✅ 미달 3건을 근거와 함께 남겼다 |
| 측정 조건이 결과와 함께 기록되어 재현 가능 | ✅ `measurements/` 각 문서에 재현 절차 있음 |
| 보완된 벤치가 저장소에 반영되고 재실행 절차 문서화 | 🟡 부분. `tools/kspon_bench`(코퍼스 채점)와 `evaluation/pipeline_bench`는 있으나 **WER·슬롯 통합 채점기와 E2E 구간 계측이 없다** |
| §9-4의 「STT WER · 핵심 표현 보존율」 행이 미측정 해제 | 🟡 부분. 공개 코퍼스는 해제, **도메인은 남았다** |
| 코퍼스로 쓴 세션 녹음이 전량 삭제되고 저장소에 남지 않음 | ⏳ **개인 음성 삭제 확인 필요** — 저장소 밖(`~/audio-test/corpus`, `~/sessions`)에 남아 있다. 커밋된 적은 없다 |

**이 판정으로 티켓을 닫는다.** 남은 세 조건은 발표(8/10~14) 이후 착수로 팀 합의돼
있으며, 후속 티켓으로 분리한다.
...
### 11-3. ✅ 안내 음성 에코가 응답으로 오인될 수 있다 — 해결됨 (S15P11A301-165), 남는 위험 1건
...
#### 검증 결과 — 로컬 실기 (2026-08-04, 세션 `20260804-055121`)

| 확인 항목 | 결과 |
|---|---|
| 안내 문구 4회 재생 중 에코 유입 | **0건** — 인식된 것은 전부 주변 사람 대화 |
| 무음 판정 실기 작동 | URGENT 턴 `raw_rms 0.0038 < SILENCE_RMS 0.005` → `NO_VOICE_DETECTED` |
| 단위 테스트 | **224개 전부 통과** (에코 가드·청취 지연 고정 테스트 포함) |

1차 방어선 `LISTEN_DELAY`(0.3초)가 실기 경로(스피커 → 공기 → 마이크)에서 유입을 막았다.
**2차 방어선(포함률 판정)이 발동할 상황 자체가 오지 않았다.** 그래서 실제 에코의 포함률
분포는 여전히 측정되지 않았고, `SENTINEL_ECHO_MATCH_RATIO` 0.9는 §11-3의 문구 실측으로만
근거를 둔 값이다.

#### 생략한 검증 2건 — 근거를 남긴다

**① INTRO 무음 시나리오(C) 실기** — 팀 판단으로 생략(2026-08-04). 무음 판정은 질문과
무관하게 같은 코드 경로(`raw_rms < SILENCE_RMS`)이며 URGENT 턴에서 실기 작동이 확인됐다.
INTRO가 무음이면 `anyResponseDetected=false`가 되는 것은 `FIELD_BY_QUESTION`에 따라
결정적이고, `false → IMMEDIATE`는 `risk_assessment()`에서 단락 처리되며 단위 테스트로
고정돼 있다. 측정 환경 제약도 사유다 — 공용 공간에서 INTRO 구간 침묵 확보가 반복 실패했다
(주변 대화가 요구조자 응답으로 잡혔다).

**② 젯슨 재인수** — 팀 판단으로 생략(2026-08-04). 165가 고친 것은 **문자열 비교**(에코
가드)와 **고정 시간 대기**(`LISTEN_DELAY`)뿐이라 기기에 따라 거동이 달라지지 않는다.
기기 의존 변수는 A2DP 싱크 버퍼 하나이며, 그것을 덮기 위해 넣은 것이 0.3초 대기다.

⚠️ **그래서 남는 위험** — **젯슨 A2DP에서 0.3초가 실제로 충분한지는 미확인이다.**
§9-3의 2026-07-30 젯슨 로그는 이 수정 **이전** 상태이므로 인수 근거로 쓸 수 없다.
부족하면 `SENTINEL_LISTEN_DELAY`를 올린다(환경변수, 코드 수정 불필요). **젯슨을 다시
켤 일이 생기면 이 값을 가장 먼저 확인한다.**
...
- `ai/voice/.gitignore`에 `sessions/`를 등록했다. 저장 위치를 저장소 안으로 지정하는
...
| 9 | **젯슨 A2DP에서 청취 지연 0.3초의 여유가 미확인이다** | 에코 가드(165)는 로컬 실기에서 유입 0건으로 확인됐으나 **젯슨 재인수는 생략했다**(근거 §11-3). §9-3의 젯슨 로그는 이 수정 **이전** 상태다. 부족하면 `SENTINEL_LISTEN_DELAY`만 올리면 되며 코드 수정은 필요 없다 |
| 10 | **실제 에코의 포함률 분포가 미측정이다** | 로컬 실기에서 유입이 0건이어서 2차 방어선이 발동할 표본이 없었다. `SENTINEL_ECHO_MATCH_RATIO` 0.9는 안내 문구 실측(§11-3)으로만 근거를 둔 값이다 |
| 11 | ✅ **입력 장치 무음이 요구조자 무응답으로 보고되던 문제** | 해결됨(257). 원인·조치·실측은 **§9-8**. 요약하면 젯슨의 PulseAudio 기본 소스가 마이크가 아닌 빈 아날로그 단자였고, 녹화와 음성 세션이 그 값을 공유했다. 이제 디지털 무음은 `AUDIO_DEVICE_ERROR`로 종료한다 |
| 12 | **기본 입력 소스가 부팅마다 고정되지 않는다** | 257의 코드 가드는 무음을 **감지**하지만 소스를 **고정**하지는 않는다. USB 재연결·다른 오디오 장치 연결로 또 어긋날 수 있고, 그때 세션은 `FAILED_AUDIO`로 끝난다(무응답 오보는 없다). 부팅 시 고정은 운영 절차·스트리밍 설정 몫이다 |
...
[178 대화 내역 저장] ✅ 완료 (§11-6)
      │
      ├──> [165 echo 가드] ✅ 완료 · 젯슨 A2DP 여유만 미확인 (§11-3, §11-7 항목 9)
      │
...
[202 세션 블랙박스 + 잡음 제거]      ← ai/voice/denoise 준비 완료 · 프라이버시 합의와
