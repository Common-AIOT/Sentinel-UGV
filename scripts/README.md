...
./scripts/viz_up.sh --local    # 127.0.0.1 만 — SSH 터널로 본다 (관제 지도는 안 나온다)
```

**이 두 스크립트는 노출을 줄이는 수단이 아니다 (S15P11A301-295).** 8765는 읽기
인증이 없고 젯슨이 공인 IP에 있어 상시 노출이며, 그것을 브릿지를 끄는 것으로
완화할 수 없다 — 끄면 관제 웹의 실시간 지도가 죽는다. 06장 위험 표와 04장 8.5가
한동안 "볼 때만 켜고 끈다"를 완화로 적고 있었고, 그 말을 믿고 시연 중에
`viz_down.sh`를 실행하면 지도를 잃는다. 실제로 범위를 좁히고 있는 것은 쓰기
차단·토픽 7개 화이트리스트·TLS이며 없는 것은 인증 하나다.

`--local`도 같은 이유로 시연 중에는 쓸 수 없다. 브라우저가 다른 기기에 있으면
루프백에 닿지 못하므로, 젯슨 자신에서 SSH 터널로 볼 때만 유효하다.

운영하지 않는 시간대에 떼어 두는 것은 여전히 유효하다.
...
이 실측 당시에는 6개였고, 이후 관제 위치 소스를 위해 `/pose/fused`가 추가돼 현재는
7개다.
...
**쓰기는 이미 막았다** — `viz.launch.py`가 `capabilities: [connectionGraph]`로
좁혔으므로(S15P11A301-224) 위 세 가지는 노출되지 않는다. 토픽도 7개
화이트리스트이고 TLS를 켜 둔다. **남은 것은 읽기 인증이 없다는 것 하나**이며,
지도·로봇 위치·스캔·TF가 그 대상이다(카메라 영상은 제외).

**그 읽기 노출은 브릿지를 끄는 것으로 완화할 수 없다 (S15P11A301-295).** 관제
웹의 실시간 지도가 이 브릿지에 직접 붙으므로 끄면 지도가 죽는다. 명시적으로
수락한 위험이며 상세는 04장 8.5·06장 36-4다.

아래 `--local` + 터널은 **개발자가 혼자 볼 때**의 경로다. 관제 웹 지도는 이 모드에서
동작하지 않으므로 시연 중에 노출을 줄이려고 쓰면 안 된다.
...
| 스크립트 | 하는 일 |
|---|---|
| `drive_measure.py` | 명령 속도·지속시간을 주고 **주행 거리**를 잰다 (S15P11A301-339) |
| `steering_measure.py` | 고정 조향으로 호를 그리게 하고 **회전 반경**을 잰다 (S15P11A301-341) |

```bash
python3 scripts/drive_measure.py 0.15 6.7          # 0.15m/s 로 6.7초
python3 scripts/steering_measure.py 0.3 4.0 22     # 0.3m/s, 4초, 바퀴각 22°(좌)
python3 scripts/steering_measure.py 0.3 4.0 -22    # 우회전
```

**두 도구는 안전 체인을 우회한다.** `/cmd_vel` 이 아니라 게이트 아래
`~/drive_command` 로 직접 쏘므로 초음파 보호정지도 Nav2 정지도 걸리지 않는다.
사람이 진로를 비우고 **12V 배터리 분리 담당자가 그 지점에 붙은 상태에서만** 쓴다 —
래칭형 물리 E-Stop 은 도입하지 않았고 하드웨어 차단은 배터리 분리뿐이다(03장 34-10).
`steering_measure.py` 는 호 **안쪽·바깥쪽을 모두** 비워야 한다. δ=22° 에서 R≈1.7m 이라
4초에 좌우로 약 1m 쓸려 나간다.

게이트 아래로 쏘는 이유는 그 위에서 측정이 성립하지 않았기 때문이다. 2026-08-09
시험에서 `/cmd_vel` 6.7초 발행 중 비영 구간이 0.60초뿐이었다 — 초음파 보호정지,
`safety_gate` 의 독립 판정, 자율 주행 명령이 같은 토픽에서 겹쳤다. 캘리브레이션은
안전 체인의 판단이 아니라 「명령한 만큼 갔는가」를 보는 작업이라, 중간 층이 값을
바꾸면 측정 자체가 무효다. 자세한 근거는 두 파일 머리 주석에 있다.

실측 결과와 그 해석은 [`docs/02-하드웨어.md`](../docs/02-하드웨어.md)·
[`docs/03-제어-캘리브레이션.md`](../docs/03-제어-캘리브레이션.md)에 있다.
