...
# 원본(정규화 전) RMS가 이보다 작으면 사실상 무음으로 판정한다.
#
# 2026-08-05 실제 세션에서 0.002998("네")와 0.004041("없습니다")가 기존
# 0.005 선게이트에 막혔지만 정규화 뒤 Silero와 로컬 Whisper small이 모두 음성으로
# 확인했다. 반면 0.000311 파일은 Whisper도 빈 전사였다(S15P11A301-303).
# 약한 요구조자 발화를 VAD까지 보내되 바닥 잡음은 막는 분리점으로 0.001을 사용한다.
SILENCE_RMS = float(os.getenv("SENTINEL_SILENCE_RMS", "0.001"))
...
# SILENCE_RMS(0.001) 사이는 세 자리 이상 벌어져 있어 진짜 무응답을 삼키지 않는다.
...
# 부상자를 무응답으로 보고한다. 근거는 docs/08-AI-음성.md 33.3.
#
