...
두 작업이 같은 이유로 막혀 있다.
  ...
  202 (잡음 제거 실기 재측정)
      ...
      동시에 녹음한 것은 1건뿐이다. 근거: `docs/measurements/잡음제거-실측.md` §9
...
`ai/stt/.gitignore`가 `sessions/`를 막고 있는 것과 같은 취급이다. **어떤 경우에도
...
def main() -> int:
    ...
    print()
    print("⚠️ 커밋하지 않는다. 측정이 끝나면 삭제한다 (docs/README.md §11-6).")
    return 0
