...
| 오도메트리·보정 | 후륜 좌·우 RS540 구동계 엔코더(센서 ESP32 경유)로 이동 거리를 측정하고, 전륜 조향각(RS380SP + MT6701)을 wheel odometry에 적용해 IMU yaw rate와 EKF로 융합한 뒤 LiDAR SLAM의 `map→odom` 보정으로 누적 오차를 줄인다. 휠베이스·트랙폭·조향각·최소 회전반경은 실측 후 확정값으로 사용한다 | 23.2, 35장 |
...
| **esp32_sensor_bridge_node** | 엔코더·DHT11·초음파 상태 중계             | /wheel_states, /odom_raw, /environment, /range/front |
...
| **분류** | **항목**                                                                 | **실패 시**            |
|----------|--------------------------------------------------------------------------|------------------------|
| 필수     | Jetson, ESP32 handshake(모터·센서 2채널), LiDAR, 카메라, 모터 드라이버, 엔코더, 전방 초음파, SLAM, Nav2, 배터리, E-Stop | 바닥 자율 탐사 시작 차단 |
...
1. 센서 ESP32가 구동축 좌·우 엔코더 tick·속도와 조향 각도(MT6701)를 제공하고, 모터 ESP32가 적용 중인 조향 상태를 제공한다.
