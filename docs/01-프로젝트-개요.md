...
|---|---|---|
| 차체 | BMW M7 유아전동차 구동 베이스 활용, 후륜 좌·우 개별 구동(RS540 FD-12V RPM14000 ×2)으로 차동 구동, 전방 수동 캐스터 바퀴 2개 지지 확정(별도 조향 모터·앞바퀴 없음, 제자리 회전 가능) | 감속비·정지 전류·트랙폭·좌우 속도 대칭은 실측 후 확정 |
| 기구 | 기존 구동부를 살리고 차체 상부·센서 브래킷을 3D 프린팅. 짐벌은 사용하지 않으며 카메라·LiDAR는 차체에 고정 마운트 | 고정 마운트 위치·높이·진동 시험 필요 |
| 컴퓨팅 | Jetson Orin Nano 8GB 고수준 컴퓨팅 + ESP32 WROOM-32 2개(모터 제어용·센서 제어용) 저수준 제어 | Raspberry Pi 5 차량 미사용 |
| 저수준 제어 | 모터 제어용 ESP32가 BTS7960 드라이버 2개(좌·우 구동)로 모터 PWM 제어, 센서 제어용 ESP32가 MT6701 엔코더 2개·차체 IMU·DHT11·HC-SR04를 수집. 두 ESP32는 각각 Jetson과 독립 USB Serial로 연결되고 각자 300ms watchdog 보유 | ESP-IDF 사용. 속도 폐루프 위치는 TBD-HW-011, IMU 모델·핀·장착 오프셋은 TBD-HW-012 |
| 관제 | AWS EC2의 Spring Boot·Next.js·PostgreSQL/TimescaleDB·S3 | Docker Compose 배포, 운영자 로그인 구현(회원가입·소셜 로그인 제외) |
| 자율주행 | ROS 2 Humble·SLAM Toolbox·Nav2·Frontier Exploration | 엔코더·IMU·LiDAR 융합과 차동 구동 운동학(트랙폭·각속도) 튜닝 필요 |
...
| 임베디드·기구 B | ESP32×2, 좌·우 구동 모터, 캐스터, 엔코더·차체 IMU, 전원, E-Stop | CAD·배선·벤치 테스트 |
...
|--------|------------------------------|----------|----------|----------------------------------------------------------------------|-------------|
| R-01   | 차동 구동 좌·우 속도 비대칭·직진 편향·캐스터 선회 저항 | 중간 | 중간 | 좌·우 게인·거리 스케일 개별 보정, 트림 적용, 캐스터 정렬·마찰 점검 | 임베디드/ROS2 |
| R-02   | 엔코더/차동 구동 기반 오도메트리 부정확 | 높음  | 높음     | IMU·LiDAR SLAM 융합, 바퀴 거리·트랙폭·각속도 스케일 보정, 속도 제한       | ROS2        |
...
| **영역**  | **완료 기준**                                               |
|-----------|-------------------------------------------------------------|
| 하드웨어  | BMW M7 후륜 차동 구동부와 전방 캐스터가 반복 주행하고 배선·전원이 안전하게 고정됨 |
| ESP32     | 좌·우 구동 엔코더 PID·차동 구동 제어, 차체 IMU 50~100Hz 원시값·측정 timestamp, CRC·sequence·300ms watchdog·부팅 안전값 검증 |
