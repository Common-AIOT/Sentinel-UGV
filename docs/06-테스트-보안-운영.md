...
> **2026-08-12 확인 (S15P11A301-377, 시연 종료 후 전수 검토).** 위 두 항목이 어떻게
> 끝났는지가 어디에도 적혀 있지 않아 다음 사람이 같은 조사를 반복하게 되어 있었다.
> 저장소에서 확정할 수 있는 것과 없는 것을 갈라 적는다.
>
> - **둘째 안(공유 토큰)은 하지 않았다 — 확정.** `SecurityConfig` 가 여전히
>   `/api/**`·`/actuator/**`·`/ws/**` 를 `permitAll()` 로 열고, 백엔드에
>   `OncePerRequestFilter`·`HandlerInterceptor`·토큰 헤더 검사가 **하나도 없다.**
>   저장소에 Nginx 설정도 없다. 즉 애플리케이션 계층에는 접근 통제가 없다.
> - **첫째 안(8080 인바운드 제한)은 저장소에서 확인할 수 없다.** Security Group 은
>   AWS 콘솔의 상태이고 이 저장소에 IaC 가 없다. 시연이 끝나 EC2 가 내려갔다면
>   이 항목은 자연히 닫힌다.
>
> **다음 사람이 할 일** — EC2 가 아직 살아 있다면 8080 인바운드부터 확인한다.
> `0.0.0.0/0` 이면 FR-025 불합격 판정이 그대로 유효하고, 인증이 없으므로
> **인터넷의 누구나 실제로 움직이는 차량에 명령을 보낼 수 있다.** 확인 결과는
> 여기에 날짜와 함께 적는다.
...
> | FR-025 (허용되지 않은 제어 없음) | **불합격으로 끝났다** (2026-08-12 재확인, S15P11A301-377). `/api/**` 가 `permitAll()` 이고 젯슨 8765 는 무인증 읽기다. 36-4 의 즉시 조치 중 **둘째(공유 토큰)는 하지 않은 것이 저장소로 확정**되고(필터·토큰 검사·Nginx 설정 전무), 첫째(8080 인바운드 제한)는 AWS 콘솔 상태라 저장소에서 확인할 수 없다. 재판정 조건과 다음 사람이 볼 것은 36-4 「즉시 조치」에 적었다 |
>
