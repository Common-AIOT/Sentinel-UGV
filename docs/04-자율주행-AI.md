...
| 항목 | 결정 | 상세 |
|---|---|---|
| 처리 파이프라인 | 센서(camera·/scan·엔코더·IMU) → EKF·TF → slam_toolbox → frontier_explorer·Nav2 → 안전 제한(collision_monitor·command_mux·velocity_smoother·safety_gate) → 고전류 모터 드라이버·조향 액추에이터(BMW M7 후륜 RS540 + 전륜 조향) | 23.1, 34-3 |
...
| 오도메트리·보정 | 후륜 RS540 구동계 엔코더로 이동 거리를 측정하고, 전륜 조향 구조가 bicycle/Ackermann으로 확인되면 실제/추정 조향각을 wheel odometry에 적용해 IMU yaw rate와 EKF로 융합한 뒤 LiDAR SLAM의 `map→odom` 보정으로 누적 오차를 줄인다. 조향 구조 확인 전에는 휠베이스·조향각·최소 회전반경을 확정값으로 사용하지 않는다 | 23.2, 35장 |
| 모터 명령 안전 체인 | 자율(Nav2)·수동(게임패드) 명령 모두 collision_monitor → command_mux → velocity_smoother → safety_gate → esp32_bridge를 거쳐 ESP32 내부 100Hz PID(Jetson과는 50Hz 명령·상태 교환)·300ms watchdog로 실행한다 | 34장(규범), 24.1 |
...
| **esp32_bridge_node**      | 모터 명령·엔코더·DHT11·초음파 상태 중계     | /wheel_states, /odom_raw, /environment, /range/front |
...
- 짐벌 구성은 안 C(LiDAR·카메라 통합 짐벌)이 확정이므로 `lidar_link`·`camera_link`는 `gimbal_pitch_link`의 자식이며, joint_state와 실제/추정 각도로 동적 TF를 발행한다. CENTER_LOCK fallback 시에는 짐벌 각도를 0으로 고정해 static TF처럼 동작한다.
...
| **분류** | **항목**                                                                 | **실패 시**            |
|----------|--------------------------------------------------------------------------|------------------------|
| 필수     | Jetson, ESP32 handshake, LiDAR, 카메라, 모터 드라이버, 엔코더, 전방 초음파, SLAM, Nav2, 배터리, E-Stop | 바닥 자율 탐사 시작 차단 |
...
flowchart TB
    ...
    gimbal_base_link --> gimbal_roll_link
    gimbal_roll_link --> gimbal_pitch_link
    gimbal_pitch_link --> lidar_link
    gimbal_pitch_link --> camera_link
```

- `map → odom`: SLAM Toolbox가 전역 보정을 제공한다.
- `odom → base_footprint`: 엔코더·IMU EKF가 연속적인 로컬 자세를 제공한다.
- 짐벌 구성은 안 C(카메라·LiDAR 통합 짐벌, 6.4장)가 확정이므로 `lidar_link`·`camera_link`는 짐벌 관절각을 반영한 **dynamic TF**를 사용한다. CENTER_LOCK fallback에서는 각도를 0으로 고정한다.
- 고정 센서(IMU 등)는 static TF를 사용한다.
...
LiDAR, 엔코더, ESP32, E-Stop, TF가 FAILED이면 자율 임무를 시작할 수 없다. 카메라가 FAILED이면 사람 탐지와 영상 기반 수동 조종을 차단하되 저속 지도 탐사 허용 여부는 운영자가 결정한다.
...
1. ESP32가 구동축 엔코더 tick·속도와 조향 상태를 제공한다.
...
flowchart TB
    ...
    gate --> bridge["ESP32 Bridge"]
    bridge --> esp32["ESP32 100Hz 제어"]
```
...
| 물리 접촉 위험 | 초근거리·범퍼·초음파 | ESP32 보호 정지 |

정지 구역은 고정 숫자만 사용하지 않고 현재 속도, 센서 갱신 지연, 300ms watchdog, 실제 제동거리를 포함해 결정한다.

초음파 임계거리 진입은 Jetson·Nav2 응답을 기다리지 않고 ESP32가 우선 정지한다. 이는 소프트웨어 기반 보호 정지이며 물리 E-Stop을 대체하거나 `ESTOP_LATCHED`로 오인하지 않는다. 정확한 경고·정지 거리는 최고 속도에서의 실차 제동 시험으로 TBD-CAL-001에 확정한다.
...
- ESP32가 유효한 `DRIVE_COMMAND`를 300ms 동안 받지 못하면 독립 정지한다.
...
- 게임패드·브라우저·MQTT·USB 중 어느 한 경로를 끊어도 300ms 이내에 ESP32 PWM이 0이 된다.
- 전방 초음파 임계거리 진입 시 Jetson bridge를 중단한 상태에서도 ESP32가 독립 정지한다.
...
이 절은 `ai/detection` 객체탐지 파이프라인을 Jetson Orin Nano 8GB에서 단독 검증하기 위한 실행 기준이다. 상세 실행 문서는 [`docs/ai/detection/jetson_runbook.md`](ai/detection/jetson_runbook.md)에 둔다.
...
- 이 wrapper는 encounter 권한이 Mission Manager에 있으므로(26.1) 토픽을 발행하지 않는다. `/perception/person_candidates`를 발행하는 `person_detector_node`(sentinel_perception)와는 별개 구현이며, 두 노드를 같은 GPU에서 동시에 상시 구동하지 않는다.
- Jetson 실기기 실측(2026-07-30): 스트리밍 스택 동시 구동 상태에서 라이브 토픽 기준 평균 11.15FPS, 디코딩 실패 0, 사람 탐지·이벤트 기록 정상.
...
Jetson Orin Nano 8GB는 SLAM, Nav2, 카메라, AI 추론이 자원을 공유한다. 성능이 부족하면 다음 순서로 조정한다.

1. `pipeline.jetson.yaml` 기본값으로 실행해 FPS와 지연을 측정한다.
2. 화면 좌상단 FPS와 `events.jsonl` 생성을 확인한다.
3. FPS가 부족하면 `pose.target_fps`, `pose.imgsz`, `detector.imgsz`를 순서대로 낮춘다.
4. 그래도 부족하면 TensorRT 변환을 검토한다.

현재 코드는 조건부 Pose를 사용한다. 사람이 3프레임 연속 감지될 때 Pose를 활성화하고, Pose는 약 2FPS로 실행하며, 사람이 3초 이상 사라지면 Pose 캐시를 중단한다. 따라서 매 프레임 Pose를 실행하지 않는다.
...
`sensor_msgs/CompressedImage`를 구독하는 wrapper는 `src/ros_main.py`로 구현되어 있다. 남은 통합 과제는 탐지 결과를 Mission Manager·cloud_bridge 경로에 연결하는 것으로, encounter 생성 권한이 Mission Manager에 있으므로(26.1) wrapper가 직접 encounter를 발행하지 않는 원칙을 유지한다.
...
- **마이크 입력: Logitech BRIO 100 내장 마이크** (짐벌 위에서 피해자를 향하므로 지향성에 유리, 추가 부품 불필요). 1주차에 주행 소음 환경 녹음 품질을 검증하고, STT 인식률 미달 시 USB 마이크로 전환한다 → TBD-AUD-001 (부록 H)
