...
|---|---|---|
| 차체 | BMW M7 유아전동차 구동 베이스 활용. **전진·후진은 후륜 좌·우 개별 구동(RS540 FD-12V RPM14000 ×2), 조향은 기존 전륜 앞바퀴 2개를 복구해 DS51150-12V 서보모터로 수행**(2026-08-06 확정). 조향 기하가 있어 최소 회전반경 제약이 있고 제자리 회전은 불가 | 감속비·정지 전류·휠베이스 `L`·최대 조향각 `δ_max`·최소 회전반경은 실측 후 확정(TBD-HW-008) |
...
- Jetson↔ESP32는 ESP32마다 독립된 유선 USB Serial을 사용하고 ESP32 Wi-Fi/Bluetooth는 안전 제어 경로에서 비활성화한다.
- 영상 경로와 제어/텔레메트리 경로를 분리한다.
- 수동 조종 연결이 끊기면 ESP32가 300ms 이내 정지한다.
- AI 탐지는 장애물 충돌 회피의 단일 안전 근거로 사용하지 않는다. 현재 충돌 안전은 LiDAR·로컬 제한·물리 E-Stop이 담당하며, 초음파는 거리 관측만 유지한다.
...
flowchart TB
    subgraph field["현장 UGV"]
        sensors["BRIO 100·X4 Pro·배터리(고정 마운트)"] --> jetson["Jetson Orin Nano 8GB\nJetPack 6.2.1+b38·ROS 2 Humble"]
...
sentinel-ugv/
├─ esp32/
│ ├─ esp32_motor/     # BTS7960 ×2 좌·우 구동 제어 펌웨어
│ ├─ esp32_sensor/    # MT6701 ×2·차체 IMU·DHT11·HC-SR04 수집 펌웨어
│ ├─ protocol/        # 두 보드 공용 프레이밍·CRC
│ └─ tests/
├─ jetson/
│ ├─ ros2_ws/src/
│ │ ├─ sentinel_bringup/
...
2. test          # 프런트엔드 타입 검사·빌드, 백엔드 시험 전체(컨텍스트 로드 포함), 메시지 계약
...
| ESP32     | 후륜 좌·우 구동 엔코더 PID, **조향 서보 중립·엔드포인트·슬루레이트 제어**, 차체 IMU 50~100Hz 원시값·측정 timestamp, CRC·sequence·300ms watchdog·부팅 안전값 검증 |
