...
| ESP32 센서 제어 | MT6701 엔코더 2개(후륜 좌·우) 계수, 차체 IMU·DHT-11·HC-SR04 수집, 초음파 근거리 감지, 통신 watchdog |
...
### `DRIVE_STATE` payload (ESP32 모터 → Jetson) — 15 bytes
...
**수동 래치는 이 규칙의 예외다** (S15P11A301-298). 위 규칙은 *젯슨이 준 모드*에 대한 것이고, 수동 래치는 **폰이 만든 것**이라 젯슨 재접속으로 풀리지 않는다. `HELLO` 는 래치를 건드리지 않으며, 그렇게 하는 이유는 젯슨 프로세스 재시작이 조작 중인 사람에게서 바퀴를 빼앗아선 안 되기 때문이다.
...
| CTRL-28 | deadman 1패킷 / 100ms·2패킷 홀드 | 전자는 PWM 0 유지·상태 불변, 후자는 `MANUAL_ACTIVE` 승격·PWM 이 `lin` 추종 |
...
**재부팅 원인은 아직 특정되지 않았다.** 모터 전류에 의한 전압 강하가 유력하다 —
주행 시에만 나는 것이 속도 의존 증상과 맞는다. 확인 방법과 펌웨어 쪽 조용한 손실
경로 셋은 S15P11A301-339에 있다.

**에일리어싱은 이 속도 범위에서 원인이 아니다.** 엔코더 샘플링이 100Hz이므로
MT6701 절대각의 모호성 한계는 샘플당 반 바퀴 = 모터축 50 rev/s이고, 0.25 m/s에서
모터축은 13~26 rev/s(유효 기어비 41 또는 82)로 한계의 절반 이하다. 이것을 적어
두는 이유는 「절대각 센서니까 빠르면 못 따라간다」가 그럴듯해서 다시 의심하게
되기 때문이다. 의심하려면 **먼저 최대 샘플 간격을 재야 한다**(339).
