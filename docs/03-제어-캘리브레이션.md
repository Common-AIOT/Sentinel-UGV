...
| ESP32 센서 제어 | MT6701 엔코더 3개(좌 구동·우 구동·조향) 계수, DHT-11·HC-SR04 수집, 초음파 근거리 감지, 통신 watchdog |
...
ESP32 WROOM-32 2개는 이 프로젝트의 100Hz 제어, MT6701 엔코더 3채널 계수, PWM, DHT-11·HC-SR04 input capture를 역할별로 나눠 처리할 성능이 충분하다. 구현은 Arduino loop 하나에 모두 넣지 않고 **ESP-IDF·FreeRTOS 기반**으로 제어·통신·센서 태스크를 분리한다.
...
| ESP32 센서 제어 → Jetson | 좌/우 구동 엔코더 tick·속도, 조향 각도(MT6701), DHT-11, HC-SR04 |
...
조건을 벗어난 값은 그대로 실행하지 않고 포화 제한하거나 거부하며 오류 코드를 반환한다. 센서 ESP32와의 통신이 끊겨 엔코더·환경·근접 데이터가 상실되어도 모터 ESP32의 자체 정지 능력에는 영향이 없지만, Jetson은 이를 오도메트리 상실로 처리해 AUTO를 `PAUSED`로 전환한다(14.5).
...
모터 ESP32의 300ms 통신 watchdog이 실제 모터 정지를 보장하므로 MQTT나 서버의 OFFLINE 판단 속도, 그리고 센서 ESP32 상태에 의존하지 않는다. 센서 ESP32가 끊기면 엔코더·환경·근접 데이터가 사라지지만, 모터 ESP32는 독립적으로 마지막 명령 기준 300ms watchdog을 유지한다.
...
100Hz 제어와 통신 watchdog은 각 보드의 고우선순위 태스크에 두고, DHT-11과 초음파는 센서 ESP32의 별도 저우선순위 태스크에서 타이머/input capture를 사용한다. 센서 응답을 기다리는 busy-wait 또는 긴 critical section으로 제어·수집 태스크를 막지 않는다.
...
| 12 | `COMM_TIMEOUT_SENSOR` | 센서 | Jetson↔센서 ESP32 통신 300ms 초과(엔코더·초음파 데이터 상실) |
...
| CTRL-20 | 센서 ESP32만 USB 분리 | 모터 ESP32는 로컬 watchdog으로 계속 안전 동작, Jetson은 엔코더·환경·근접 데이터 상실을 감지해 AUTO를 PAUSED 전환 |
...
차체 상태 추정용 IMU 1개(`imu_link`)만 사용한다. 짐벌이 없으므로 별도의 head IMU는 필요하지 않다.
