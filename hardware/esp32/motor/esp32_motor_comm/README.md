...
- `DRIVE_STATE`(50Hz)/`DIAGNOSTIC`(5Hz) 송신(placeholder 값)

포함하지 않음(후속 PWM 티켓):
- BTS7960 실제 PWM/DIR 구동 — `safety_stub.h`의 `applySafeOutputs()`/`applyDriveTargets()`가 그 훅
- 전류/과열 등 실제 드라이버 fault 판독
...
- `safety_stub.h/.cpp` — 후속 티켓이 채울 실제 액추에이션 훅.
