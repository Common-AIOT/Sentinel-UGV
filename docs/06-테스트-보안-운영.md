...
| 벤치 테스트         | 구동 바퀴·앞바퀴를 띄운 상태에서 검증 | 후륜 좌·우 구동 방향, 조향 스윕·중립·엔드포인트, watchdog, E-Stop |
...
## 16.2 하드웨어 테스트 체크리스트
차량을 바닥에서 띄운 상태에서 시작한다(38-6 환경 E1). ESP32 부팅 시 PWM 0·enable LOW, USB 분리 후 300ms 이내 정지, E-Stop 시 모터 전원 차단, 프로그램 종료 시 정지는 38장 CTRL 시험군과 G0~G1 Gate에서 검증한다. 모터 ESP32의 Wi-Fi는 모바일 수동 조종에만 사용하고 자율·E-Stop·watchdog 경로에는 넣지 않는다. Bluetooth는 사용하지 않는다. DHT11·초음파 timeout이 100Hz 센서 태스크를 막지 않는지도 확인한다. 시험 ID가 없는 물리 점검 항목은 다음과 같다.
...
| FR-023 | ESP32 firmware ×2(모터·센서), esp32_motor_bridge, esp32_sensor_bridge | CTRL-01~27 | 100Hz 제어, 300ms watchdog, IMU 50~100Hz·측정 timestamp, 부팅 안전값, 초음파 거리 관측·중계 코드·E-Stop, **조향 서보 스윕·슬루레이트·정지 시 각도 유지**(CTRL-24~27). 조향 경로는 구현됐고 초음파 보호 정지는 현재 발동 비활성 | 펌웨어 로그·rosbag·영상 |
