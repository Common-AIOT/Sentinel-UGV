...
정황이 맞는다: 2026-08-08 `drive_burst.py 0.15 6.7`(명령 기대 약 1.0m)에서 실제
...
**아직 재측정 전이다.** 플래시 후 실주행으로 확인할 것은 셋이다.

1. 실제 루프 주기가 10ms 근처인지
2. `drive_burst.py 0.15 6.7` 이 100cm 근처로 오는지 (수정 전 183cm)
3. 전진·후진에서 `/wheel/odometry` 부호가 정상인지

이 셋이 확인되기 전까지 위의 「2026-08-08 실측은 근거로 쓰지 않는다」는 그대로 유효하다.
