...
| `RETRY_UNCLEAR` | `guide_retry_unclear.wav` | 목소리가 잘 들리지 않았습니다. 천천히 다시 말씀해 주세요. | COUNT·MOBILITY·URGENT 이해 실패 시 1회 재질문(§11-5) |
...
1. **`SILENCE_RMS = 0.005`는 현장 무음 방어선이 아니다.** 소음 RMS 0.006 이상에서
   ...
   취급한다. 상세: 실측 문서 §4-3.
2. **`STT_PROMPT`(도메인 프라이밍)가 환각을 만든다.** 로컬 `small`이
   `"다쳤어요, 다쳤어요, 가스, 화재"` — `config.STT_PROMPT` 자체를 그대로 반환한
   사례가 실제 녹음에서 확인됐다. Jira 120의 `initial_prompt` 제거 A/B는 방향이 정해졌다.
...
포함률 분포에 빈 구간이 있다. 안내 문구와 꼬리 조각은 **1.00**으로 모이고, 실제 응답은
최대 **0.78**이다.

| 들린 말 | 포함률 | 가장 가까운 안내 문구 |
|---|---|---|
| 승인 문구 10개 전부 | **1.00** | 자기 자신 |
| `"말이 들리면 대답해"` (꼬리) | **1.00** | INTRO |
| `"잠시만 기다려 주세요"` (꼬리) | **1.00** | REPORT_PENDING |
| `"스스로 움직일 수 있어요"` | **0.78** | ASK_MOBILITY |
| `"지금 움직일 수 있어요"` | 0.62 | ASK_MOBILITY |
| `"숨쉬기 어렵고 피가 나요"` | 0.56 | ASK_URGENT |
| `"두 명이고 움직일 수 없어요"` | 0.30 | ASK_MOBILITY |
| `"다리를 다쳐서 못 움직여요"` | 0.10 | ASK_MOBILITY |

⚠️ **`"스스로 움직일 수 있어요"`가 0.78인 이유** — `ASK_MOBILITY`가 "지금 **스스로
움직일 수** 있나요?"라고 묻기 때문에, **요구조자가 질문의 단어를 그대로 써서 답한다.**
처음 고른 임계값 0.8에서는 여유가 0.02뿐이었고, 그러면 **걸을 수 있는 부상자가
`anyResponseDetected=false` → `IMMEDIATE`로 올라간다.** 안전한 방향의 오류지만 다수
사상자 상황에서 구조 우선순위를 잘못 배분하고, 이동 가능 답변 자체가 사라진다.
그래서 **0.9**로 올려 양쪽에 0.1 이상 여유를 뒀다.

최소 길이 **8자**(`SENTINEL_ECHO_MIN_CHARS`)도 같은 이유다. `"네"`·`"살려주세요"`·
`"피가 많이 나요"` 같은 짧은 응답은 길이에서 걸러 판정 대상에 넣지 않는다.
...
**이 상황을 위한 승인 문구가 이미 있었다.** `RETRY_UNCLEAR`("목소리가 잘 들리지
않았습니다. 천천히 다시 말씀해 주세요.")가 자산으로 등록돼 있는데 **코드가 한 번도
쓰지 않았다.** 배선만 했다(§2-1의 재질문 표).

> 이것은 183의 `REPORT_SUCCEEDED_DEPARTURE`와 같은 모양이다 — 승인된 자산이 잠긴 채
> 놀고 있었다. 새 녹음이 필요 없었다.
...
| `small` 모델의 한계다 | 정규화·프롬프트 어느 쪽도 `움직일`→`이럴`을 고치지 못함 | 🟡 후보. **젯슨 지연·RAM 측정 필요**(120) |
...
[178 대화 내역 저장] ✅ 완료 (§11-6)
      ...
      │           └──> [180 VAD 보조 게이트] (§11-5, 에코 가드 뒤에만 감도 상향 가능)
      │
