...
#: `collision_monitor` 의 전방 정지 구역 0.40m 보다 **크게** 잡는다 — 그보다 작으면
#: 안전 체인이 먼저 속도를 0 으로 만들어 이 모듈은 영원히 도착을 선언하지 못한다.
DEFAULT_STOP_DISTANCE_M = 0.60

#: 24.2 「피해자 접근」 행. 이 값은 여기와 안전 체인이 함께 지킨다.
#:
#: **0.10 이었다가 0.15 로 올렸다** (2026-08-08, S15P11A301-343). 0.10 명령은
#: PWM 17% 로 정지 마찰을 못 넘어 **로봇이 움직이지 않는다** — 접근이 시작조차
#: 안 되고 도착 선언이 없어 INTERACTING 으로 넘어가지 못한다(실측: 0.10 정지,
#: 0.15 주행. 경계는 PWM 17~25% 사이, S15P11A301-342). 0.15 는 움직이는 최저
#: 대역이므로 이보다 내리면 안 된다. PWM 매핑(342) 확정 후 체감 속도를 재확인한다.
DEFAULT_MAX_SPEED_MPS = 0.15
