...
**2026-08-06 정정 (S15P11A301-295): "볼 때만 켜고 보고 나면 끈다"는 완화가 아니다.** 이 자리에 그렇게 적혀 있었으나 적용할 수 없다 — 관제 웹의 실시간 지도가 이 브릿지에 직접 붙으므로(04장 8.6, `LiveMap.tsx`) 끄면 지도가 죽는다. `--local` + SSH 터널도 같은 이유로 시연 중에는 쓸 수 없다. 실측: `enable_viz` 기본값이 `true`이고 서비스가 enabled이라 **부팅만으로 8765가 `0.0.0.0`에 열린다.** 즉 노출 구간은 스택이 떠 있는 동안 전부다. 자세한 내용은 04장 8.5.
...
> | FR-023 (IMU 50~100Hz·측정 timestamp) | MPU6050·측정 timestamp·`/imu/data_raw` 약 90Hz 경로 구현 완료. 센서 태스크 주기 수정도 반영됐고 bias·공분산·90°/360° 회전 재계측이 남음 |
...
> 현재 차단 사유는 소비자 부재가 아니라 기본 비활성 스위치, 조향 링키지 보정,
> 센서 재계측과 실차 인수 미완료다.
...
| SR-002 | 물리 E-Stop 독립 차단 | CTRL-08 | Jetson·ESP32 오동작과 무관하게 모터 전력과 조향 서보 전원 차단. **E-Stop 후 조향각은 정의되지 않는다**(서보 무여자, 03장 34-7) |
...
|---|---|
| 라이다 정지·감속 구역 (`collision_monitor`, 전방 0.40m/0.75m) | 명령이 `/cmd_vel_muxed` 를 거치지 않는다(04장 24.1) |
...
**남는 보호**: 물리 E-Stop, 모터 ESP32 fault·통신 워치독, BTS7960 드라이버 fault,
초음파 중계의 순간 정지. 즉 **수동 조종은 사람이 보면서 하는 조작이라는 전제 위에
있다.** 관제 화면(StatusPanel)에도 이 사실을 한 줄로 표시한다.
...
- 물리 E-Stop 후 조향각은 정의되지 않는다 — 서보 전원이 함께 끊겨 앞바퀴가 자유로워진다(03장 34-7). 차단점을 하나로 유지하기로 한 결정의 대가다.
...
- [ ] 물리 E-Stop과 ESP32 300ms watchdog 통과(구동 전원 + 조향 서보 전원 동시 차단)
...
| **SR-002**  | 물리 E-Stop은 소프트웨어와 무관하게 모터 전원과 조향 서보 전원을 차단해야 한다. |
...
4. 차량을 띄운 상태에서 엔코더 방향·IMU `/imu/data_raw` 주기·모터 PID 시험, **조향 서보 중립·엔드포인트 스윕 확인**
