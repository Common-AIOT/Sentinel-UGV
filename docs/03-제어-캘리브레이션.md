...
> Sentinel UGV는 Jetson이 생성한 선속도·각속도를 **전륜 조향 기준의 후륜 목표 속도와 조향각**으로 변환해 유선 USB Serial로 모터 ESP32에 전송한다. 모터 ESP32는 BTS7960 ×2와 DS51150-12V 조향 서보를 구동하고, 센서 ESP32는 MT6701 ×2·MPU6050·DHT-11·HC-SR04 ×2를 수집한다. Jetson은 `/imu/data_raw`와 wheel odometry를 발행해 EKF로 융합한다. 두 ESP32 사이에는 직접 배선이 없어 전방 초음파 보호 신호를 Jetson 모터 브리지가 `STOP_COMMAND`로 중계한다. 후방 거리는 관측만 한다. 유효 명령이 300ms 동안 없으면 모터 ESP32가 정지하며, 물리 E-Stop은 BTS7960 전원과 조향 서보 전원을 함께 차단한다.
...
4. RPP에는 유효한 `minimum_turning_radius` 파라미터가 없으므로 설정에 임의로 넣지 않는다. 원주행 결과는 `vehicle_kinematics`의 조향 상한 검증과 NavFn 유지/Smac Hybrid-A\* 전환 판단에 사용한다(24.1).
