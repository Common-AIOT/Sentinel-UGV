...
### VID-01 — 동일 Wi-Fi WebRTC 지연 30회 (2026-07-30)

**합격.** 동일 Wi-Fi의 Windows 노트북 브라우저에서 측정했습니다.

```text
표본       30회 (1초 간격, 접속 후 5초 워밍업 제외)
중앙값     138.4ms   기준 ≤350ms
P95        338.4ms   기준 ≤500ms
최소/최대  65.2 / 507.5ms
RTT 중앙값 114.0ms
FPS 평균   29.9
```

측정 공식은 관제 웹 `useWhepStream`과 같습니다 —
`totalProcessingDelay/framesDecoded + RTT/2`. 다른 점 두 가지가 있고 이유가
있습니다.

- **워밍업 5초를 버립니다.** 접속 직후 jitter buffer가 안정되기 전의 값은 정상
  상태의 지연이 아닙니다.
- **구간 증분으로 계산합니다.** `totalProcessingDelay`는 누적값이라 누적 평균을
  쓰면 워밍업 구간이 30회 내내 섞여 실제보다 나쁘게 나옵니다.

젯슨이 같은 인증서(S15P11A301-145)로 측정 페이지를 HTTPS 서빙하고, 노트북
브라우저가 열면 30회 수집 후 결과를 자동 회신하는 방식을 썼습니다. 관제 웹 화면을
사람이 읽어 적는 것보다 정확하고 재현 가능합니다.

32-9의 스트리밍 검증 항목 중 마지막 미검증이었습니다.
