...
|---|---|---|
| 차체 | BMW M7 유아전동차 구동 베이스 활용, 후륜 좌·우 개별 구동(RS540 FD-12V RPM14000 ×2, e-diff) + 전륜 조향(WW MOTOR RS380SP-12V RPM5500 ×1) 확정 | 감속비·정지 전류·조향 링크 백래시·최소 회전반경은 실측 후 확정 |
...
| 저수준 제어 | 모터 제어용 ESP32가 BTS7960 드라이버 3개(좌·우 구동, 조향)로 모터 PWM 제어, 센서 제어용 ESP32가 MT6701 엔코더 3개·차체 IMU·DHT11·HC-SR04를 수집. 두 ESP32는 각각 Jetson과 독립 USB Serial로 연결되고 각자 300ms watchdog 보유 | ESP-IDF 사용. 속도 폐루프 위치는 TBD-HW-011, IMU 모델·핀·장착 오프셋은 TBD-HW-012 |
...
| ESP32 ×2               | 모터 PWM·조향·브레이크와 엔코더·차체 IMU·환경·근접 센서 원시값 수집, 측정 timestamp·sequence, 300ms watchdog, fault | 경로 계획, 객체 인식, IMU EKF, 클라우드 통신 |
...
sentinel-ugv/
├─ esp32/
│ ├─ esp32_motor/     # BTS7960 ×3 구동·조향 제어 펌웨어
│ ├─ esp32_sensor/    # MT6701 ×3·차체 IMU·DHT11·HC-SR04 수집 펌웨어
...
| 임베디드·기구 B | ESP32×2, 모터·조향모터, 엔코더·차체 IMU, 전원, E-Stop | CAD·배선·벤치 테스트 |
...
| ESP32     | 구동 엔코더 PID·조향 제어, 차체 IMU 50~100Hz 원시값·측정 timestamp, CRC·sequence·300ms watchdog·부팅 안전값 검증 |
