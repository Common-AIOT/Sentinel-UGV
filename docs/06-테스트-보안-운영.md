...
- □ 좌·우 구동 모터 방향과 조향 모터(RS380SP) 중앙·좌우 한계 확인
- □ 배선 발열·커넥터 이탈 없음(BTS7960 ×3 포함)
- □ Jetson 저전압/재부팅 없음
...
6. 회원가입은 제외하고 관리자가 사전 등록한 운영자 로그인은 구현한다. 조종 API는 허용 네트워크·배포 환경 토큰·Control Session 없이 공개하지 않는다.
...
→ REST/WSS 연결
→ 연결된 게임패드가 있는 하나의 브라우저 세션만 Control Session 획득
→ Control Session과 유효 TTL이 있는 명령만 주행
```
...
- 브라우저 종료·네트워크 단절·게임패드 해제 시 Control Session 해제 및 STOP
...
flowchart TD
    ...
    time --> devices["카메라·LiDAR·ESP32·IMU 확인"]
...
4. udev 별칭으로 카메라·LiDAR·ESP32·IMU 장치를 확인한다.
5. ESP32 handshake와 펌웨어 버전을 확인한다.
...
> **MVP 범위 외 (2026-07-28, TBD-OPS-001)** 아래 8개 중 구현된 유닛은 **없다.**
> MVP는 자동 복구를 구현하지 않으므로 `Restart` 정책이 주는 이점이 사라지고,
> 남는 것은 부팅 시 자동 시작이라는 편의뿐이다. 개발 중에는 오히려 방해가 된다
> (로그가 `journalctl`로 가고, `Restart`가 죽인 프로세스를 되살려 디버깅이 꼬인다).
>
> 이 절은 향후 설계로 유지한다. 38장 인수 시험에 systemd를 요구하는 항목은 없다.
> 현재 실행은 사람이 `scripts/start_sentinel.sh`로 한다.
>
...
- `esp32-motor-bridge`가 죽어도 모터 ESP32 watchdog이 모터를 정지한다. `esp32-sensor-bridge`가 죽으면 엔코더·환경·근접 데이터가 끊겨 Jetson이 AUTO를 PAUSED로 전환한다.
...
| `FR-023` | Jetson과 ESP32 2개(모터 제어용·센서 제어용)를 분리하고, 모터 ESP32가 저수준 모터 구동을, 센서 ESP32가 엔코더·DHT11·초음파 수집·watchdog을 담당해야 한다. | 필수 |
...
| FR-015 | telemetry, dashboard | TEL-01 | 온습도·배터리·Jetson·ESP32 상태 2Hz 표시 | 화면·DB |
...
| FR-023 | ESP32 firmware ×2(모터·센서), esp32_motor_bridge, esp32_sensor_bridge | CTRL-01~21 | 100Hz 제어, 300ms watchdog, 부팅 안전값, 초음파 보호 정지(Jetson 중계)·E-Stop 시험 통과 | 펌웨어 로그·영상 |
...
| SR-006 | 모터(구동·조향)·Jetson 전원 분리 | HW-01 | 배선도·정격 검토와 부하 시험 통과 |
...
| **FR-023** | ESP32가 모터·엔코더·DHT11·초음파·300ms watchdog을 담당해야 한다. | 필수     |
...
| **SR-006**  | 구동·조향 모터 전원은 Jetson 전원과 적절히 분리해야 한다.         |
