...
void setup() {
  ...
  // §34-6: 전원 인가 직후 PWM=0·driver enable=LOW를 먼저 보장하고, 조향의 내부
  // 목표만 중립(δ=0)으로 준비한다. 실제 서보 펄스는 아래의 주변장치 초기화가 끝난
  // 뒤 제어 태스크에서 시작한다. 통신 태스크는 액추에이터를 직접 구동하지 않는다.
  // 조향 목표만 중립으로 시작하는 것은 재부팅 직후에 믿을 수 있는 마지막
  ...
  // steeringInit()은 PWM duty 0인 disarmed 상태로 끝난다. 모든 주변장치 초기화가
  // 끝난 지금 공유 상태만 arm하고, 아래에서 시작하는 유일한 액추에이터 태스크가
  // 첫 50Hz 갱신 때 기본 중립 145°에 해당하는 펄스를 곧바로 출력하게 한다.
  // 0°/1500µs 같은 임시 펄스를 거치지 않으므로 종료 전 같은 중립에 있었다면 큰
  // 재이동이 없다. setup() 앞 구간은 GPIO18 외부 10kΩ pull-down이 LOW로 유지한다.
  motorSharedStateUpdate([](MotorSharedState& s) { s.servoArmed = true; });
