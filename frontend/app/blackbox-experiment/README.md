...
> **개발 전용이다** (S15P11A301-377). 이 파일이 `app/` 아래에 있어서 Vercel 운영
> 도메인의 `/blackbox-experiment` 로 그대로 열렸다. 목 데이터로 도는 참조 구현이
> 실제 관제와 같은 주소 공간에 있으면 링크를 받은 사람이 진짜 화면인지 알 수 없다.
> 이제 운영 빌드에서는 `notFound()` 로 404 다. `npm run dev` 에서는 그대로 열린다.

> **복사본이 낡았다** (2026-08-12 실측). 아래 「이관 전에 diff」 절차대로 재면
> 원본에만 있는 줄이 **399줄**, 이 파일에만 있는 줄이 **171줄**이다. 원본이
> 그만큼 앞서 갔다는 뜻이므로, 이관할 때는 토글 블록만 떼어 원본에 얹고 이 파일의
> 나머지는 참고하지 않는다.
