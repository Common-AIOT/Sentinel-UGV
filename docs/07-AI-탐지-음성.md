...
>
> **v2.0-r1: 구 04-자율주행-AI.md 에서 분리해 신설했다(S15P11A301-245).** 9·25·30·33장 —
> AI 인식, 사람 탐지, 피해자 오케스트레이션, 음성 상호작용. 이 영역(객체탐지·STT)은
> 구현이 거의 완료되어 참조 문서 성격이다. 자율주행 장(8·14·22·23·24·26)은
> [04-자율주행.md](04-자율주행.md)에 있다. 장 번호는 문서 전체에서 전역이다.
...
거리를 얻는 값싼 대안도 있다. **방위만으로 접근**(bearing-only) — 그 방향으로 정렬하고 초음파·LiDAR 가 임계 거리라고 할 때까지 0.10 m/s 로 전진한다. Nav2 목표조차 필요 없고 24.2 의 접근 속도와 24.3 의 "사람 접근 중 안전거리보다 가까워지면 목표와 무관하게 정지"([04-자율주행.md](04-자율주행.md))와 그대로 맞는다. `ESTIMATED` 위치만 있을 때의 대비책으로 쓴다.
...
#### `events/` 보존 상한 (S15P11A301-265)

`events/*.jpg` 는 31-5 봉투의 `eventImage` 가 가리키는 **증빙**이므로 끄지 않는다.
대신 상한을 두고 오래된 것부터 버린다.

상한이 없으면 총량이 **장면에 따라** 결정된다. `event_cooldown_seconds`(15초)가
**트랙별**이므로 사람 1명당 15초이고, 6명이 상시 잡히면 24 이벤트/분이 된다.
실제로 run 하나가 **1.2G** 까지 커져 젯슨 디스크가 찼다.

| 항목 | 값 | 근거 |
|---|---|---|
| 장당 크기 | **165KB** | 1280×720, `cv2.imwrite` 기본 화질, 실물 프레임 12장 중앙값(최소 163·최대 168KB) |
| `events_max_bytes` | 500MB (약 3,100장) | 디스크 여유 5.6G·하루 3 run 이면 세 run 이 다 차도 1.5G |
| `events_max_files` | 5000 | **보조 안전망이며 지금은 발동하지 않는다** — 165KB 기준으로 바이트 상한이 먼저 걸린다 |

**개수가 아니라 바이트가 주 기준인 이유**는 장당 크기가 장면 복잡도와 overlay 유무에
따라 달라지기 때문이다. 개수로 묶으면 총량이 장면에 따라 다시 흔들린다. 막고 싶은
것이 디스크이므로 디스크를 직접 잰다.

**보존 시간으로 환산하지 않는다.** 6명이 상시 잡히는 사무실에서는 2.2시간 분이고
요구조자가 한두 명인 시연 상황에서는 며칠 분이다. 상한의 목적은 "며칠 치를 남긴다"
가 아니라 **"장면이 어떻든 디스크가 차지 않는다"** 다.

삭제는 개수·용량과 함께 stderr 로 남긴다. 조용히 지우면 나중에 "왜 그 시각 증빙이
없나" 에 답할 수 없다 — 이것은 캐시가 아니라 증빙 데이터다. 기동 시에도 한 번
정리하는데, 저장 시점에만 검사하면 이전 run 이 남긴 초과분이 그대로 남는다.
...
| LLM | **GMS 클라우드** | `ai/voice/sentinel_voice/llm.py` 가 `openai` 클라이언트로 `GMS_BASE_URL` 호출, 기본 `gpt-5.4-mini`. 구조화 해석 전용 |
