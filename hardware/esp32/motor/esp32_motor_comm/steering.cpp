...
//
// **-1 은 실측이다** (2026-08-09 호 주행 시험, S15P11A301-341). δ=+22° 지령에
// 로봇이 우회전했다 - IMU yaw 적분 -37.1°(시계방향) + 육안 확인 일치. IMU 축은
// 344 기능 시험으로 검증된 상태라(반시계=gz+, 적분 +92°/실측 90°) 신뢰 근거가
// 겹으로 있다. +1 이던 동안 Nav2 의 모든 조향 명령이 거울상이었다 - 좌로
// 피하라는 명령에 우로 꺾었다. 회전 반경 크기는 이 시험에서 이미 정확했다
// (실측 R 1.71m / 기대 1.69m, 비 1.01 - 링키지 비 2.5 반영 확인).
