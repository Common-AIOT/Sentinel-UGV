...
| 항목 | 결정 | 상세 |
|---|---|---|
| 처리 파이프라인 | 센서(camera·/scan·엔코더·IMU) → EKF·TF → slam_toolbox → frontier_explorer·Nav2 → 안전 제한(collision_monitor·command_mux·velocity_smoother·safety_gate) → vehicle_kinematics(차동 구동) → BTS7960 ×2(BMW M7 후륜 좌·우 RS540 ×2, 전방 수동 캐스터 ×2) | 23.1, 34-3 |
...
| 탐사 종료·복귀 | 도달 가능한 Frontier가 10초간 발견되지 않으면 일시정지하고, 제자리 회전 또는 짧은 전·후진 재관측 기동으로 LiDAR 재스캔을 수행한다. 재스캔 후에도 Frontier가 없으면 탐사 완료로 판단한다. 최대 탐사 시간 7분, 사용자 종료, 배터리 20% 이하도 종료 조건이다. 종료 시 mission_manager가 저장한 home pose를 Nav2 목표로 전송하고, 복귀 경로 생성 실패 시 PAUSED로 전환해 관제에 복구 요청을 표시한다 | 23.4~23.5 |
...
| **esp32_motor_bridge_node** | 좌·우 구동 명령 전송, 모터 상태 중계  | /drive_state |
| **esp32_sensor_bridge_node** | 엔코더·IMU·DHT11·초음파 상태 중계         | /wheel_states, /odom_raw, /imu/data_raw, /environment, /range/front |
| **vehicle_kinematics_node** | v/ω ↔ 좌·우 구동 속도 변환(차동 구동)   | /cmd_vel_safe → esp32_motor_bridge |
...
차동 구동 로봇이므로 global planner는 최소 회전반경 제약이 없는 NavFn 또는 Smac 2D를 우선 검증하고, local controller는 DWB(또는 차동 모드의 Regulated Pure Pursuit)를 기본 후보로 사용한다. 제자리 회전이 가능하므로 목표 heading 정렬용 rotation shim을 활성화하고, 경로와 recovery에 제자리 회전과 전·후진 선회를 모두 활용한다.
