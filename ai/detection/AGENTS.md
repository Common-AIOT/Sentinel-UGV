...
### 명세 이탈 0건 — 해소된 이력 — 2026-08-12 재감사

아래 내용은 사유·영향·되돌리는 법의 기록으로 남긴다.
새로 합류하는 사람이 "명세와 코드가 왜 다른가"를 물을 때 이 절이 답이다.

> **2026-08-12:** Pose crop 이 명세 25.2 에 기술되면서 **남은 이탈이 없다.**
> 2026-08-11 에 명세가 BoT-SORT 로 정정되어 추적기 이탈이 해소됐고, ReID 는 최종
> 시연 범위에서 제외되어 두 프로파일 모두 꺼져 있다.
...
**해소됨. Pose를 전체 화면이 아니라 person crop으로 실행한다 (2026-07-30)**

**2026-08-12 해소.** 종전에는 명세가 Pose 를 "어디에" 돌리는지 말하지 않아 구현의
선택이 기록되지 않은 상태였다(어긋난 문장이 있던 것이 아니라 문장 자체가 없었다).
`docs/07-AI-탐지.md` 25.2 에 「Pose 실행 범위 — 전체 화면이 아니라 person crop」 절을
세워 crop·좌표 복원·전역 예산을 명세에 올렸다. 아래는 그 결정의 사유 기록이다.
...
되돌려야 할 상황이 오면 각 항목의 "되돌리는 법"을 쓴다. 승인 내용은 2026-08-12 에
명세 25.2 로 올라갔으므로, 근거를 찾을 때 이 문서가 아니라 명세를 먼저 봐도 된다.
...
- **2026-08-05 팀 공지 완료.** 관제 화면이 이 값을 표시하게 될 때 2값 기준으로 만든다.
...
| 10b | **부동(inactivity) 신호 3개는 미검증** — `inactivity_boost`, `motion.still_ratio`, `motion.full_still_seconds`. E-FPDS가 정지 이미지라 측정에서 빠졌다. 2026-08-06 팀 판단으로 **알려진 한계로 수용**했고, 2026-08-12에 근거를 명세 25.2에 정리하며 **열린 검증 과제에서 뺐다**(25.7). 틀리는 방향이 미탐 쪽이고, 젯슨 실기 2건에서 오탐이 관측되지 않은 것이 근거다. 오탐·미탐이 보고되면 여기부터 본다.<br>**검증 수단 정정** — 종전에 "영상 입력 도구를 새로 만들어야 한다"고 적혀 있었으나 사실이 아니다. `--frame-log`가 `frames.jsonl`의 `signals.inactivity`에 trackId별 시계열을 이미 남긴다. 없는 것은 도구가 아니라 **정답 구간 표기**이며, `data/pose_test/`의 클립에 구간을 찍으면 고정 시점 분리도까지는 잴 수 있다. 재현율은 UGV 촬영 영상(2b)이 있어야 한다 | 수용된 한계 | — | §15, `../../docs/07-AI-탐지.md` 25.2 |
