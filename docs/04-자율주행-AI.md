...
detector:
  ...
  imgsz: 640
  quantize: 16       # FP16. Jetson 실측으로 확정할 것 (아래 성능 조정 순서)
...
```json
"stage_ms_per_frame": {"detect": 296.6, "pose": 15.1, "post": 0.9},
"stage_share":        {"detect": 0.81,  "pose": 0.18, "post": 0.004},
"pose_ms_per_run": 131.28
```

개발 PC 계측(2026-07-30, 사람 약 4명)에서는 **Detect가 81%** 였다. 이 비율이 Jetson에서도 유지되면 Pose 쪽 조정은 효과가 작고 Detect의 TensorRT 변환이 가장 큰 지렛대다. Jetson에서 같은 계측을 다시 해 확인한다.

**2단계 — A/B 벤치.** `ai/detection/scripts/bench_jetson.py`가 설정 조합을 순차 실행하고 표로 비교한다.

```bash
python scripts/bench_jetson.py \
  --source <기준영상> \
  --config configs/pipeline.jetson.yaml \
  --fp16 --warmup 30
```
...
- 비교 축: `--fp16`, `--models`(TensorRT 엔진 비교), `--imgsz`, `--pose-budget`.
...
**4단계 — 조정 순서.** 정확도를 깎지 않는 것부터 쓴다.

1. **FP16 확인** — `detector.quantize: 16`의 유/무를 A/B한다. 현재 값은 미검증이며, 개발 PC(x86)에서는 오히려 4배 느렸다(19.69 → 4.86 FPS).
2. **TensorRT 변환** — Jetson에서 직접 구워야 한다(크로스 빌드 불가, 장치·드라이버 버전 종속).
   ```bash
   yolo export model=models/yolo26n.pt format=engine half=True device=0
   ```
   설정에서 `detector.model`을 `.engine` 경로로 바꾸면 끝이다. 코드 변경이 없다. 엔진은 정밀도가 이미 구워져 있으므로 이때 `quantize` 비교는 무의미해진다. 엔진 파일은 `.gitignore`가 막고 있으니 커밋하지 않는다.
3. **전원 모드** — 15W 상향은 SLAM·Nav2와 자원·발열을 나누므로 **팀 합의가 필요하다.**
4. **해상도 축소** — `detector.imgsz` 640 → 512 → 416. 25.5가 허용한 범위지만 위 세 가지를 소진한 뒤에 손댄다. 한 단계씩 내리며 매번 `detections` 회귀를 확인한다.
...
위 "보행자 4명 1280x720 600프레임" 영상이 저장소에 없다. 성능 A/B는 고정 영상으로만 재현 가능하므로, 이 영상의 소재 또는 동등한 대체 영상 확보가 성능 작업의 선행 조건이다.
