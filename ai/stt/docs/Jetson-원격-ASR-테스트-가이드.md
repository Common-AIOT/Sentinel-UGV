...
| Jetson STT 환경 파일 | `/home/orin/projects/S15P11A301/ai/stt/.env` |
...
```bash
test -f ai/stt/requirements-jetson-remote.txt
test -f scripts/jetson_voice_preflight.sh
test -f ai/stt/docs/Jetson-원격-ASR-테스트-가이드.md
```
...
```bash
pgrep -af '^/home/j-i15a301/a301-gpu-asr/.venv/bin/python -m gpu_server$'
...
```bash
ASR_PID="$(pgrep -f '^/home/j-i15a301/a301-gpu-asr/.venv/bin/python -m gpu_server$')"
printf 'ASR_PID=%s\n' "$ASR_PID"
test -n "$ASR_PID"
test "$(printf '%s\n' "$ASR_PID" | wc -l)" -eq 1
```
...
```bash
.venv/bin/python -m pip install -r ai/stt/requirements-jetson-remote.txt
...
파일 위치는 `/home/orin/projects/S15P11A301/ai/stt/.env`다.

```bash
cd /home/orin/projects/S15P11A301/ai/stt
...
증적: /home/orin/projects/S15P11A301/ai/stt/results/jetson-preflight/<UTC시각>
```
...
프로세스가 바로 종료되면 `.venv`, `rclpy`, `ai/stt/.env`부터 다시 확인한다.
...
```bash
cd /home/orin/projects/S15P11A301/ai/stt
../../.venv/bin/python -m bench.jetson_resource_bench \
