...
- `sentinel_perception`의 `person_detector_node`(S15P11A301-136)는 ai/detection 완성 전의 임시 통합 구현으로, person_candidates 발행 역할이 이 wrapper로 이관되었다(S15P11A301-153). 메인 인식 로직은 항상 ai/detection이며, 두 YOLO 노드를 같은 GPU에서 동시에 상시 구동하지 않는다.
