...
| 항목 | 결정 | 상세 |
|---|---|---|
| 처리 파이프라인 | 센서(camera·/scan·엔코더·IMU) → EKF **[구현]** → TF → slam_toolbox **[구현]** → frontier_explorer·Nav2 → 안전 제한(collision_monitor·command_mux·velocity_smoother·safety_gate) → vehicle_kinematics(차동 구동) **[미구현, 8.2]** → BTS7960 ×2(BMW M7 후륜 좌·우 RS540 ×2, 전방 수동 캐스터 ×2) **[구현]**. IMU 는 MPU6050 으로 확정돼 `/imu/data_raw` 가 90Hz 로 오고 EKF 도 붙었다(S15P11A301-244·236). 다만 **기판 장착 방향이 미확정**이라 기본은 꺼짐이며, 그때는 휠 오도메트리만 쓴다 | 23.1, 34-3, 8.2 |
...
>
> **연속 실패가 최대 1회라 `ENVIRONMENT_SENSOR_FAULT`(임계 3)는 뜨지 않는다.** 진단에
> 흔적이 남지 않는 것을 알고 수용하는 것이며, 실패율이 크게 오르면 연속 3회에 걸려
> 그때는 진단이 잡는다.
>
...
얼어붙는다. 기동 시점 판단으로는 닿지 않는 런타임 문제이며, S15P11A301-237
안전 체인에서 TF 끊김을 정지 조건으로 다룰 때 함께 잡는다.
