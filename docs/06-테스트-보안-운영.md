...
> | FR-011 (입력 단절 후 300ms 이내 PWM 0) | ROS 측 `safety_gate` 는 구현됐고 상류 단절 시 `/cmd_vel=0` 을 20Hz 로 유지하는 것까지 확인했다. 초음파 `protective_stop` → `STOP_COMMAND` 중계도 구현했다(둘 다 S15P11A301-237, 03장 「중계 구현」). 남은 것은 **ESP32 까지의 실차 계측** — 300ms 안에 PWM 이 0 이 되는지는 재지 않았다 (TBD-HW-011, CTRL-18) |
...
| FR-023 | ESP32 firmware ×2(모터·센서), esp32_motor_bridge, esp32_sensor_bridge | CTRL-01~27 | 100Hz 제어, 300ms watchdog, IMU 50~100Hz·측정 timestamp, 부팅 안전값, 초음파 보호 정지(Jetson 중계)·E-Stop 시험 통과, **조향 서보 스윕·슬루레이트·정지 시 각도 유지**(CTRL-24~27). 조향 경로는 **미구현** — 04장 8.2 갭 표 | 펌웨어 로그·rosbag·영상 |
...
| 전방 초음파(HC-SR04) 근거리 보호 | 센서 ESP32 에 달려 있고 모터 보드로 직접 배선이 없다(TBD-HW-011). 젯슨 중계 `STOP_COMMAND` 는 **순간 정지**뿐이며 다음 앱 패킷이 다시 굴린다(re-arm 규칙으로 완화하나 완전하지 않다) |
...
- **전륜 서보 조향 차량이므로 제자리 회전을 할 수 없다.** 최소 회전반경보다 좁은 공간에서 방향을 바꾸려면 전·후진을 섞은 3점 선회가 필요하고, 후진 구간에는 초음파 보호가 없다(04장 23.6).
