...
/**
 * 관제 블랙박스 화면 **실험판** — 소음 제거본 토글 (S15P11A301-202).
 *
 * `app/blackbox/page.tsx`를 그대로 복사한 뒤 두 가지만 바꿨다.
 *
 *   ① import 가 실제 `api` 대신 `./mockApi` — 백엔드·S3 없이 띄우기 위한 것뿐이다
 *   ② 「소음 제거본 토글」 로 주석 표시한 부분 — 이관 대상 기능
 *
 ...
 * ## 토글이 이렇게 생긴 이유
 *
 * 브라우저는 MP4 안의 오디오 트랙 전환을 지원하지 않는다(Chrome `audioTracks` 미지원).
 * 그래서 제거본을 별도 `<audio>`로 두고 어느 쪽 소리를 낼지만 바꾼다. 제거본은
 * 원본 오디오에서 파생되므로 타임라인이 표본 단위로 일치하고, 아래 동기 코드는
 * 재생·탐색·배속을 따라가기 위한 것이다.
 *
 * 근거·실측: ai/stt/docs/measurements/잡음제거-실측.md §5
 */
