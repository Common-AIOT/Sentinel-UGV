...
| 저수준 제어 | 모터 제어용 ESP32가 BTS7960 드라이버 2개(좌·우 구동)로 모터 PWM 제어와 **조향 서보 PWM 생성**을, 센서 제어용 ESP32가 MT6701 엔코더 2개·차체 IMU·DHT11·HC-SR04를 수집. 두 ESP32는 각각 Jetson과 독립 USB Serial로 연결되고 각자 300ms watchdog 보유 | ESP-IDF 사용. 조향은 서보 내부 폐루프이므로 외부 각도 피드백 없음(개루프). 속도 폐루프 위치는 TBD-HW-011, IMU 모델·핀·장착 오프셋은 TBD-HW-012 |
...
- 차체 IMU 1개, DHT11, 초음파(HC-SR04 ×2, 전방·후방)는 센서 제어용 ESP32가 직접 읽는다. 후방은 2026-08-08 착수(S15P11A301-324, TBD-HW-010). 별도의 head IMU는 사용하지 않으며, 초음파 근거리 정지는 Jetson 응답을 기다리지 않고 로컬에서 우선 적용한다.
...
13. Frontier 소진, 7분 경과 또는 사용자 종료 시 신규 탐사를 중단한다. (배터리 기반 종료는 v2.0 에서 폐기 — 04장 14.6)
