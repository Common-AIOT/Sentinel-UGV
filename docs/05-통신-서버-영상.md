...
- **게임패드 제어 — 폐기됨(S15P11A301-298).** 아래 매핑은 관제 웹에 조종 입력이 있다고 전제했으나 그 UI 는 삭제됐고(28장), 조종 수단은 폰 → 모터 ESP32 직결로 확정됐다. 남겨 두는 것은 이력이며 **구현 대상이 아니다.** 특히 `B=일반 정지` 는 이제 모바일 페이지의 ■정지 버튼이고, 그것은 바퀴만 세울 뿐 **MANUAL 을 벗어나지 않는다**; `연결 해제=수동 종료` 도 성립하지 않는다 — 수동을 벗어나는 길은 관제 「자율」 하나뿐이다(04장 14.2). 실제 조종 계약은 31-13 의 HTTP 엔드포인트 표다.
  ...
  수동 조종은 관제 PC에 연결된 USB 또는 Bluetooth 게임패드만 사용하고, 브라우저 Gamepad API로 입력을 읽으며, 장치가 없으면 수동 모드 버튼을 비활성화하고 연결 안내를 표시한다. 입력 매핑은 왼쪽 스틱 상하=전진/후진(deadzone·최대 속도 제한), 왼쪽 스틱 좌우=좌/우 **조향**(각속도·각가속도 제한), LB=데드맨 스위치(누르고 있을 때만 DRIVE 전송), B=일반 정지(즉시 0 속도), Start=출발지 복귀 요청(확인 후 RETURNING), 연결 해제=수동 종료(300ms 이내 ESP32 정지 후 PAUSED)로 확정한다. UX 절차와 위험 조작 보호는 28.4~28.5를 따른다.
  </details>

> **2026-08-06 전륜 서보 조향 전환: 좌우 스틱만으로는 로봇이 회전하지 않는다.** 종전 매핑은 "제자리 회전 포함"이었으나 조향이 서보로 바뀌어 **전진·후진 중에만 회두가 생긴다**(02장 6.3). 좌우 스틱 단독 입력(`linear≈0`, `angular≠0`)은 Jetson 운동학 계층에서 거부되고 진단 경고만 남는다(03장 34-2). 프런트엔드는 이 상황을 조작자에게 알려야 한다 — 아무 반응이 없으면 게임패드나 연결을 의심하게 된다. 최소 조치는 **`linear` 가 deadzone 안인데 `angular` 입력이 있을 때 「전진하면서 조향하세요」 안내를 띄우는 것**이며, 조향각 자체는 정규화 `angular` 값을 Jetson 이 `δ` 로 변환하므로 프로토콜(31-6)은 바뀌지 않는다.
...
```json
{
  ...
  "data": {
    ...
    "health": {"mcuConnected": true, "lidarOk": true, "cameraOk": true, "recorderOk": true, "recorderLastFailure": null},
...
>
> 백엔드 적재와 화면 표시는 아직 없다. `health`의 두 필드를 저장·노출하는 것은
> S15P11A301-309에 위임돼 있다.
