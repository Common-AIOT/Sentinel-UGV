...
| USB Serial ×2    | Jetson↔ESP32 모터 제어(목표 속도·조향각·fault), Jetson↔ESP32 센서 제어(엔코더·DHT11·초음파) | native CDC/USB-UART, CRC·sequence·300ms timeout |
...
| Jetson ↔ ESP32 ×2 | 유선 USB Serial 우선(모터·센서 채널 분리) | 구동 속도·조향각 목표·오류·E-Stop 상태(모터), 엔코더·조향각·DHT11·초음파(센서) |
