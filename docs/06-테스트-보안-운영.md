...
>
> **안전은 영향받지 않는다.** 아래 정책의 두 번째 항목대로 `esp32-bridge`가 죽어도
> ESP32 watchdog이 모터를 정지시킨다. systemd는 그 근거가 아니다.

권장 서비스:

```text
sentinel-esp32-bridge.service
...
- `esp32-bridge`가 죽어도 ESP32 watchdog이 모터를 정지한다.
...
```bash
systemctl --failed
journalctl -u sentinel-esp32-bridge -f
...
| `FR-023` | Jetson과 ESP32를 분리하고 ESP32가 저수준 모터·엔코더·DHT11·초음파·watchdog을 담당해야 한다. | 필수 |
| `FR-024` | 짐벌은 NAV_LOCK과 CENTER_LOCK을 제공하고 SLAM 악화 시 fallback해야 한다. | 필수 |
...
| FR-023 | ESP32 firmware, bridge | CTRL-01~19 | 100Hz 제어, 300ms watchdog, 부팅 안전값, 초음파 보호 정지·E-Stop 시험 통과 | 펌웨어 로그·영상 |
...
| **FR-024** | 짐벌은 NAV_LOCK/CENTER_LOCK과 fallback을 제공해야 한다.     | 필수     |
