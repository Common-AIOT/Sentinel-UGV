...
flowchart TD
    ...
    kin --> bridgeM["esp32_motor_bridge 약 20Hz"]
    ...
    espS --> bridgeS
    bridgeS -.엔코더·오도메트리 관측.-> kin
    estop["물리 E-Stop (미도입)"] --> driverL
    estop --> driverR
    estop --> servo
    estop --> espM
```
...
| 구성 요소 | 최종 책임 |
|---|---|
| Jetson | SLAM·Nav2·YOLO·임무 상태, 수동/AUTO 명령 선택, 장애물 안전 제한, 목표 `v/ω`를 **후륜 목표 속도와 전륜 조향각 `δ`로 변환**(전륜 조향, 34-2). 엔코더는 EKF 오도메트리에 쓰며 구동 속도 PID에는 쓰지 않음 |
| ESP32 모터 제어 | BTS7960 2개 PWM/DIR 제어(전진·후진), **조향 서보 PWM 생성·엔드포인트 클램프·슬루레이트 제한**, 통신 watchdog, 오류 시 출력 차단 |
| ESP32 센서 제어 | MT6701 엔코더 2개(후륜 좌·우), MPU6050, DHT-11, HC-SR04 ×2(전방·후방) 수집, 초음파 보호 정지 판단 코드(현재 발동 비활성), 통신 watchdog |
...
| ~~물리 E-Stop~~ | **미도입(설계만) — 34-10 참조.** 하드웨어 차단은 12V 배터리 연결 분리뿐이다 |

이 구조는 Linux 프로세스 지연과 CUDA·SLAM 부하가 모터 PWM 생성에 직접 영향을 주지 않게 한다. 두 ESP32 모두 경로를 계획하거나 AI 판단을 하지 않으며, 유효한 목표 값만 안전하게 실행·수집한다.

### ESP32 적용 평가와 제약

ESP32 WROOM-32 2개는 이 프로젝트의 100Hz 제어, MT6701 엔코더 2채널 계수, PWM, 차체 IMU 100Hz 수집, DHT-11·HC-SR04 input capture를 역할별로 나눠 처리할 성능이 충분하다. 펌웨어는 **Arduino-ESP32 스케치 위에서 FreeRTOS 태스크를 분리**한다.
...
- ESP32는 교육용 프로토타입 제어기이며 안전 인증 제어기가 아니다. 외부 pull-down 은 계속 독립적으로 유지한다(물리 E-Stop 은 미도입 — 34-10).
...
```text
1. ~~물리 E-Stop~~ (미도입 — 34-10. 실질 최상위는 2번이다)
...
| `0x10` | `DRIVE_COMMAND` | Jetson → ESP32 모터 | 모터 | 약 20Hz |
...
| `0x21` | `DIAGNOSTIC` | ESP32 → Jetson | 모터/센서 | 5Hz 주기(오류 즉시 전송 없음) |
...
| `0x30` | `CONFIG_GET/SET` | 양방향 | 모터/센서 | 스텁(키 테이블·모드 게이팅 미구현) |
...
flags                    u8   # ENABLE, BRAKE, CLEARABLE_STOP 예약(현재 생성·소비 안 함)
...
command_timeout_ms       u16  # 수신·보관만 함. 실제 watchdog은 고정 300ms
```
...
rear_min_distance_mm  u16   # 후방 HC-SR04 거리, invalid면 0xFFFF
valid_sensor_mask     u8    # bit0=전방 유효, bit1=후방 유효
protective_stop       u8    # 전방 판정 필드. 현재 PROXIMITY_STOP_ENABLED=false라 항상 0
sample_age_ms         u16
```

payload는 8바이트로 구현돼 있으며 후방 값과 `valid_sensor_mask` bit1이 Jetson `/range/rear`까지 전달된다. 후방 센서는 `TRIG=GPIO5`, `ECHO=GPIO36`이다. `protective_stop` 필드와 중계 경로는 호환성을 위해 유지하지만, 현재 펌웨어는 전방 오측 때문에 `PROXIMITY_STOP_ENABLED=false`로 발동을 막아 항상 0을 보고한다. 후방은 애초에 판정식에 포함되지 않는다.

DHT-11 실패는 `DEGRADED` 환경 텔레메트리로 처리하고 주행을 바로 중단하지 않는다. 보호 판정을 다시 켜면 HC-SR04 임계거리 진입 시 센서 ESP32가 로컬에서 `protective_stop`을 세우고, Jetson 모터 브리지가 모터 ESP32에 `STOP_COMMAND`를 중계한다. 현재는 센서 거리 측정·발행과 중계 코드만 유지되고 입력 신호가 항상 `false`라 실제 정지가 발생하지 않는다. 활성화 전에는 전방 센서 오측 제거와 왕복 지연 실측이 필요하며, 현재 최종 안전 수단은 LiDAR `collision_monitor`·모터 ESP32 fault·watchdog 경로다(TBD-HW-011). 물리 E-Stop 은 미도입이라 이 목록에 없다(34-10).
...
`PROXIMITY_STATE` 가 10~20Hz 로 오므로 매 프레임 중계하면 20Hz `DRIVE_COMMAND` 와
...
펌웨어 판정 상수는 `PROXIMITY_STOP_DISTANCE_MM=100`(0.10m, 전방 전용)이고,
실제 장착 상태에서 보호 정지가 관측된 거리는 **0.15~0.18m**였다(04장 22.2 실측표).
로봇 반경이 0.20m 이므로 이 신호가 뜨는 시점에는 물체가 이미 차체 외곽선 안에 있다.

**주 방어는 LiDAR·`collision_monitor`**(정지 구역 범퍼 앞 0.25m — 실측 차체 기준, 04장 24.1)이고 이 중계는
그 뒤에 오는 층이다. 정상 상황에서는 라이다가 먼저 멈추므로 이 경로가 발동할 일이
없다. 발동했다면 라이다가 놓친 것이며, 목적은 충돌을 없애는 것이 아니라 **충격을
줄이는 것**이다. 최종 근거리 안전은 `collision_monitor` 와 모터 ESP32 자체 fault 판정이
받는다.

> **2026-08-09 발동 비활성 (S15P11A301-353).** 전방 센서가 빈 공간에서 2.6~5.5cm
> 오측을 간헐적으로 내(15초 218표본 중 9회, 커넥터 분리 시 0회) 주행이 반복해서
> 끊겼고, 시연 요구는 라이다 층으로 충족된다는 판단으로 **발동 판정만 펌웨어에서
> 껐다**(`sensor_task.cpp` `PROXIMITY_STOP_ENABLED = false`). 측정·발행·중계·게이트
> 코드는 전부 그대로다 — 신호가 영구히 `false` 일 뿐이다. 전방도 후방(TBD-HW-011)과
> 같은 「측정만 하는」 지위가 됐다.
>
> **받아들인 리스크**: 라이다 평면(z=0.50) 아래 낮은 장애물은 이제 아무 층도 못
> 본다. 사람 다리·의자는 라이다에 잡히므로 교실 시연 기준 수용. 되살리는 조건은
> 353 완료 기준(빈 공간 15초 오측 0회) 통과 후 플래그 복원이다.
...
2. 통신 watchdog을 시작한다. IWDT/TWDT는 Arduino 프레임워크 기본 idle 감시만 있고 제어 태스크를 명시 등록하지 않았다.
...
| ESP32 모터 IWDT/TWDT | 프레임워크 기본 idle 감시만 사용 | 제어 태스크 미등록으로 태스크 정지 시 MCU reset은 **미보장** |
...
- ~~물리 E-Stop~~: **미도입**(34-10). 배터리 연결 분리가 이를 대신하며, 재연결 후에도 자동 재출발은 없다
...
**전원을 끊는 경우(배터리 분리, 또는 도입 시 물리 E-Stop)에는 이 정책이 성립하지 않는다.** 서보 전원이 함께 끊겨 무여자되므로(6.6) 앞바퀴가 노면 반력에 따라 움직일 수 있다. 즉 E-Stop 후의 조향각은 정의되지 않으며, 이것은 차단점을 하나로 유지하기로 한 결정의 대가다(21.4).

전원을 다시 연결했다고(또는 도입 시 물리 E-Stop 을 해제했다고) 차량이 자동으로 움직이면 안 된다. 모터 ESP32와 Jetson 양쪽에서 `E_STOP_RESET`과 새로운 운전 시작 명령을 확인한 뒤에만 enable하며, 이때 조향은 중립부터 시작한다.
...
| 작업 | 주기 |
|---|---:|
| 후륜 구동 엔코더 I2C 읽기(좌/우 MT6701, IMU와 같은 태스크) | 100Hz 목표 |
| `ENCODER_STATE` 송신 | 50Hz |
| MPU6050 I2C 읽기 | 100Hz 목표 |
| `IMU_STATE` 송신 | 약 90Hz 실측 |
| 전방 초음파 측정·보호 판단 | 약 15Hz 설정 |
| 후방 초음파 측정·관측(전방 다음에 순차 trigger) | 약 15Hz 설정, 보호 정지 미연결 |
| DHT-11 측정 | 0.5~1Hz |
| 통신 watchdog 검사 | 1kHz |

구동 속도 제어는 **엔코더 PID가 아닌 개루프 피드포워드**다. 모터 ESP32가 실측 회귀식 `PWM = 13.3 + 184.8·|v|`로 mm/s 명령을 PWM으로 변환하며, 0.30m/s 명령에서 실속도 99%를 확인했다(S15P11A301-342). 센서 ESP32의 엔코더는 오도메트리·EKF에만 쓰고 Jetson이 속도 오차를 모터 명령에 반영하는 폐루프는 미구현이다. 경로 추종 오차는 Nav2 상위 폐루프가 보정한다.
...
- 목표 속도 포화 제한
- **150mm/s 미만 속도 데드밴드**(해당 명령은 PWM 0)
- 조향각 `±δ_max` 클램프와 변화율 제한(엔드포인트를 기계 한계보다 안쪽에 둔다)
- `abs(v) < v_min`에서 조향 목표 변경 금지(34-2)
- 방향 전환 전 짧은 중립·감속 구간

PWM 변화율 제한, 엔코더 신선도·좌우 속도 편차, BTS7960 fault, 과전류·저전압 보호는 **미구현**이다(해당 입력 센서도 없음).
...
| Bit | 코드 | 보고 채널 | 의미 | 검출 상태 |
|---:|---|---|---|---|
| 0 | `COMM_TIMEOUT_MOTOR` | 모터 | Jetson↔모터 ESP32 명령 300ms 초과 | 구현 |
| 1 | `CRC_ERROR_RATE_HIGH` | 모터/센서 | 해당 채널 직렬 프레임 오류율 초과 | 정의만 |
| 2 | `DRIVE_ENCODER_FAULT` | 센서 | 후륜 좌/우 MT6701 무응답·비정상 | 구현 |
| 3 | `WHEEL_SPEED_MISMATCH` | 센서 | 좌·우 속도 편차 | 정의만 |
| 4 | `DRIVER_FAULT` | 모터 | BTS7960 fault 입력 | 정의만(입력 핀 없음) |
| 5 | `OVERCURRENT` | 모터 | 전류 임계값 초과 | 정의만(센서 없음) |
| 6 | `UNDERVOLTAGE` | 모터 | 구동 전원 저전압 | 정의만(센서 없음) |
| 7 | `ESTOP_ACTIVE` | 모터 | **소프트웨어 `ESTOP_COMMAND` latch.** 물리 E-Stop은 전원을 끊어 보드가 관측하지 못함. 펌웨어 `fault_codes.h` 주석이 이 비트를 「모터: 물리 E-Stop 입력」으로 적고 있으나 **그런 입력 핀은 없다** — 코드를 먼저 읽는 사람이 부품 존재를 오인하므로 임베디드 담당이 주석을 고친다(34-10) | 구현(latch 전송자 없음 — 05장 11.5) |
| 8 | `CONFIG_INVALID` | 모터/센서 | 설정값 오류 | 정의만 |
| 9 | `INTERNAL_WATCHDOG_RESET` | 모터/센서 | IWDT/TWDT 재부팅 이력 | 정의만 |
| 10 | `PROXIMITY_SENSOR_FAULT` | 센서 | 초음파 timeout·비정상값 지속 | 구현 |
| 11 | `ENVIRONMENT_SENSOR_FAULT` | 센서 | DHT-11 checksum·timeout | 구현 |
| 12 | `COMM_TIMEOUT_SENSOR` | 센서 | Jetson↔센서 ESP32 300ms 초과 | 구현 |
| 13 | `IMU_SENSOR_FAULT` | 센서 | IMU bus 오류·비정상 | 구현 |
| 14 | `STEERING_COMMAND_INVALID` | 모터 | 범위 밖 조향 명령 거부 | 구현 |
| 15 | `STEERING_RESPONSE_MISMATCH` | 모터/Jetson | 기대 yaw rate·IMU yaw rate 간접 비교 | 미구현 |
...
- 모터 시험은 구동 바퀴를 바닥에서 띄운 상태에서 시작한다. 조향 시험도 앞바퀴를 띄운 상태에서 엔드포인트를 먼저 확인한다.

> **⚠️ 물리 E-Stop 은 도입하지 않은 채 프로젝트가 끝났다 (2026-08-11 확인).**
>
> 위 도식의 「래칭형 NC E-Stop」은 **설계이며 실물이 없다.** 부품 모델·차단 정격이
> 끝까지 확정되지 않았고(TBD-HW-007) 시연 차량에도 장착되지 않았다. 발표자료의
> 하드웨어 구성도에도 E-Stop 이 없다.
>
> **그래서 이 장과 다른 장이 「안전 우선순위 최상위」로 적어 온 층은 실재하지 않는다.**
> 실제로 동작한 정지 수단은 아래 넷이며 전부 소프트웨어·전원 계통이다.
>
> | 수단 | 발동 조건 | 근거 |
> |---|---|---|
> | 관제 임무 정지·일시정지 | 운영자 조작 | 26장 |
> | 수동 조종 deadman | 폰 입력이 250ms 끊기면 정지 | 펌웨어 `MANUAL_TTL_DEFAULT_MS` |
> | Jetson 명령 watchdog | 유효 명령이 300ms 없으면 정지 | 펌웨어 `JETSON_WATCHDOG_TIMEOUT_MS` |
> | collision_monitor 정지 구역 | LiDAR 가 구역 침범을 보면 정지 | 04장 24.1 |
>
> **최후 수단은 12V 모터 배터리 연결을 손으로 분리하는 것이다** — 전용 스위치가 없어
> 차단점이 배터리 커넥터 자체다. 이때도 서보가 무여자되어 조향각이 정의되지 않는 것은
> 34-7 서술과 같다. 소프트웨어 `ESTOP_COMMAND` latch(34-9 fault bit 7)는 물리 차단과
> 무관한 별개 층이며, 그것이 이 프로젝트에서 「E-Stop」이라 불린 실체다.
...
└─ esp32_bridge/
   ...
   ├─ protective_relay.py               # 초음파 보호 정지 중계
...
| ~~CTRL-08~~ | ~~물리 E-Stop~~ | **수행 불가 — 부품 미도입**(34-10, TBD-HW-007). 도입 시 검증할 내용: Jetson·ESP32 상태와 무관하게 모터 전력과 조향 서보 전원이 함께 차단되고, 서보 무여자 후 앞바퀴가 자유로워지는 것을 확인·기록(정의된 거동, 34-7) |
...
| CTRL-18 | 전방 초음파 임계거리 진입 | 거리 측정과 `/range/front` 발행은 계속 검증한다. `protective_stop` 중계와 단위 시험은 구현됐지만 `PROXIMITY_STOP_ENABLED=false`라 현재 신호는 항상 0이며 실제 정지 시험은 수행할 수 없다. 빈 공간 15초 오측 0회 확인 뒤 플래그를 복원하고, 코드 임계 0.10m·종전 장착 발동 0.15~0.18m에서 PWM 0까지 왕복 지연을 실측한다 |
| CTRL-18R | 후방 초음파 거리 진입 | `/range/rear` 거리와 `valid_sensor_mask` bit1이 정확히 변하고 `protective_stop`에는 영향을 주지 않음을 확인한다. 후방 관측 경로는 구현됐으며, 보호 정책을 채택한다면 전방과 별도로 임계·정지거리를 기록한다 |
...
| CTRL-23 | IMU 분리·I2C 오류 | `IMU_SENSOR_FAULT`, 오래된 샘플 EKF 입력 차단, AUTO PAUSED 또는 명시적 축소 운용 |
...
> Sentinel UGV는 Jetson이 생성한 선속도·각속도를 **전륜 조향 기준의 후륜 목표 속도와 조향각**으로 변환해 유선 USB Serial로 모터 ESP32에 전송한다. 모터 ESP32는 BTS7960 ×2와 DS51150-12V 조향 서보를 구동하고, 센서 ESP32는 MT6701 ×2·MPU6050·DHT-11·HC-SR04 ×2를 수집한다. Jetson은 `/imu/data_raw`와 wheel odometry를 발행해 EKF로 융합한다. 두 ESP32 사이에는 직접 배선이 없어 전방 초음파 보호 신호를 Jetson 모터 브리지가 `STOP_COMMAND`로 중계한다. 후방 거리는 관측만 한다. 유효 명령이 300ms 동안 없으면 모터 ESP32가 정지한다. **물리 E-Stop 은 미도입으로 끝났고**(34-10) 하드웨어 차단은 12V 배터리 연결 분리뿐이다.
...
종전:  30 / 30 = 1.0   (가정값 — 바퀴가 지령의 40%만 꺾였다)
현재:  55 / 22 = 2.5   (2026-08-09 펌웨어 반영: steering_limits.h 22000,
                        steering.cpp·board_state.h 55, S15P11A301-341)
```

검증은 `scripts/steering_measure.py` 로 한다 — 고정 조향 호 주행에서
R = 경로길이/yaw변화 를 자동 산출하며, δ=22° 에서 R≈1.69m 면 반영, ≈4.4m 면
미반영이다.

**2026-08-09 호 주행 실측: 반경은 정확, 방향은 반대였다.** δ=+22° 에 실측
R 1.71m(기대 1.69m, 비 1.01)로 링키지 비 반영이 확인됐으나 **yaw 가 −37°
(우회전)** 이었다 — 서보를 올리면 바퀴가 우로 꺾이는 실물 링키지라
`SERVO_DIRECTION_SIGN` 을 −1 로 바꿨다(코드 주석이 예고해 둔 그 항목, CTRL-24).
+1 이던 동안 Nav2 의 모든 조향이 거울상이었다.

**부호 수정 후 좌·우 최종 실측 (2026-08-09, v=0.3, 4초 호 주행):**

```text
방향        yaw 변화     실측 R    기대 R    실측/기대    역산 δ
좌 (+22°)   +43.6°       1.37 m    1.69 m    0.81         26.5°
우 (−22°)   −36.6°       1.76 m    1.69 m    1.04         21.2°
```

방향 부호는 양쪽 다 지령과 일치한다. **좌회전이 우회전보다 약 20% 급하게
돈다** — 부호 반전 전 같은 물리 방향(서보 반대 극)의 측정이 1.71m 였으므로
재현되는 기하 비대칭이다(링키지 지오메트리·서보 혼 각도·중립 미세 오프셋 후보).
Nav2 모델(1.69m) 기준으로 우회전은 4% 넓게, 좌회전은 19% 급하게 도는데 **둘 다
안전한 방향**이다 — 계획한 곡률보다 못 도는 쪽(4.4m 시절)이 문제였지, 더 도는
것은 폐루프가 흡수한다. 좌·우 대칭 정밀화는 TBD-CAL-001 의 기존 항목으로 남긴다.

덤으로 얻은 운영 지식: **모터 보드는 플래시 직후 전원 재인가가 필요할 수 있다.**
플래시만 하면 TX(DIAGNOSTIC)는 살고 RX 가 죽어 HELLO 무응답·DRIVE_STATE 침묵이
되는 상태를 두 번 겪었고, 두 번 다 전원 재인가로 해소됐다.
...
4. RPP에는 유효한 `minimum_turning_radius` 파라미터가 없으므로 설정에 임의로 넣지 않는다. 원주행 결과는 `vehicle_kinematics`의 조향 상한 검증과 Smac Hybrid-A\*의 `minimum_turning_radius=1.8` 확정 근거로 사용했다(24.1).
...
정방향과 역방향 스케일을 비교하고, 좌·우 스케일 차이가 크면 타이어, 구동계 백래시, 엔코더 방향, 개루프 속도→PWM 매핑과 좌·우 트림부터 점검한다.
...
**이 시점의 중간 결론은 명령 대비 실제 속도가 남았다는 것이었다.** 0.20m/s 명령에서
실속도가 **약 38~40%**로 두 측정에 일관됐고(7.6 / 8.0cm/s), 바로 아래 추가 실측에서
운용점 0.25~0.30m/s를 확인해 최종 결론을 갱신했다.
...
끊김 없음) `driver_enabled` 도 참이었다. 저속 차이는 명령 단절이 아니라 정지 마찰
구간의 비선형성이었고, S15P11A301-342에서 운용 대역을 0.25~0.30m/s로 고정해 닫았다.
...
**안전 체인을 우회하므로 시험 전용이다.** 앞을 비우고 12V 배터리 커넥터에 손이 닿는(전용 E-Stop 미도입 — 34-10)
상태에서만 쓴다.
...
MPU6050 1개(`imu_link`)를 센서 ESP32의 GPIO21/22 I2C 400kHz 버스에 연결한다. 짐벌이 없으므로 별도의 head IMU는 사용하지 않는다. 센서 ESP32는 원시 gyro·accel과 monotonic 측정 시각을 전달하고, Jetson sensor bridge가 `sensor_msgs/Imu` `/imu/data_raw`로 변환한다.
...
| 사람 접근 속도 | **0.25m/s** (종전 0.10 — 데드밴드 아래라 실속도 0, S15P11A301-368) | 안전거리 시험 |
...
**저속 접근에서 각속도 상한이 실질적으로 사라진다.** (아래 수치는 종전 상한 0.10m/s 기준이며, 그 값은 펌웨어 데드밴드(150mm/s) 아래라 실제로는 로봇이 움직이지 않아 0.25 로 고쳤다 — S15P11A301-368. 0.25m/s·R_min 1.69m 면 각속도 상한은 0.15rad/s 로, 아래 논지는 그대로 유효하다.) 사람 접근 0.10m/s에서 `R_min`이 1m라면 가능한 각속도는 0.1rad/s이며, 표의 0.3rad/s는 물리적으로 나오지 않는다. 접근 제어를 설계할 때 이 제약을 전제해야 하고, 좁은 공간에서 사람을 정면으로 보려면 **여러 번의 전·후진 선회(3점 선회)** 가 필요하다.
...
```text
jetson/ros2_ws/src/sentinel_bringup/config/
├─ safety.yaml          # 운동학 L·δ_max·속도, smoother·collision monitor
├─ nav2.yaml           # Smac·RPP·footprint·R_min
├─ ekf.yaml            # wheel odometry·IMU 융합
├─ slam_toolbox.yaml
├─ brio_100.yaml
└─ ydlidar_x4_pro.yaml

hardware/esp32/motor/esp32_motor_comm/
├─ steering_limits.h   # δ_max
└─ steering.cpp       # 서보 중립·엔드포인트·매핑
```
