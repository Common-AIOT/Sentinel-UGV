...
| E-Stop 상태 | 상태 표시만                | 물리 E-Stop은 차량에서 조작 |
...
- 소프트웨어 E-Stop 채널 이름은 `cmd/estop`과 `/app/robots/{robotId}/estop`으로 설계돼 있으나 **현재 생산자·구독자·관제 버튼이 없어 미사용**이다. 실제 차단은 물리 E-Stop과 모터 ESP32 fault 경로가 담당한다.
...
| E-Stop | 관제는 상태만 표시. 물리 버튼과 모터 보드가 실제 차단을 담당 |
...
| `GET` | `/api/v1/telemetry/latest?robotId=...` | 로봇 최신 텔레메트리 조회 | 구현 |
