...
현재 구현은 **USB 카메라를 직접 열어 단독 검증**하는 단계까지 준비되어 있다. 최종 로봇 통합에서는 9.6장과 32-3장 원칙에 따라 AI 프로세스가 카메라를 직접 열지 않고, `usb_cam` ROS2 노드가 발행하는 `/camera/image_raw/compressed`를 구독하는 wrapper로 감싸야 한다.
...
이 절의 실행 방식은 USB 카메라 직접 실행용 단독 검증이다. 최종 통합 구조는 다음 흐름을 따른다.
...
즉 최종 통합에서는 `cv2.VideoCapture(0)` 대신 ROS2 `sensor_msgs/CompressedImage`를 구독하는 wrapper가 필요하다.
