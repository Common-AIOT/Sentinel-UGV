...
| 저수준 제어 | 모터 제어용 ESP32가 BTS7960 드라이버 3개(좌·우 구동, 조향)로 모터 PWM 제어, 센서 제어용 ESP32가 MT6701 엔코더 3개·DHT11·HC-SR04를 수집. 두 ESP32는 각각 Jetson과 독립 USB Serial로 연결되고 각자 300ms watchdog 보유 | ESP-IDF 사용. 정확한 GPIO 핀 배정과 속도 폐루프 위치(TBD-HW-011)는 진행 |
...
## 3.1 핵심 설계 원칙
- 영상·LiDAR·AI·주행 판단은 Jetson에서, 모터·엔코더·통신 watchdog은 ESP32 2개(모터 제어용·센서 제어용)에서, 관제·명령·이력은 EC2에서 처리한다.
- DHT11과 전방 초음파는 센서 제어용 ESP32가 직접 읽는다. 초음파 근거리 정지는 Jetson 응답을 기다리지 않고 로컬에서 우선 적용한다.
...
flowchart TB
    subgraph field["현장 UGV"]
        sensors["BRIO 100·X4 Pro·IMU·배터리(고정 마운트)"] --> jetson["Jetson Orin Nano 8GB\nJetPack 6.2.1+b38·ROS 2 Humble"]
...
│ ├─ esp32_sensor/    # MT6701 ×3·DHT11·HC-SR04 수집 펌웨어
...
| 임베디드·기구 B | ESP32×2, 모터·조향모터, 엔코더, 전원, E-Stop | CAD·배선·벤치 테스트 |
