...
| 구성 요소 | 최종 책임 |
|---|---|
| Jetson | SLAM·Nav2·YOLO·임무 상태, 수동/AUTO 명령 선택, 장애물 안전 제한, 목표 `v/ω`를 좌·우 구동 속도로 변환(차동 구동, 34-2), 두 ESP32의 엔코더·명령 채널을 결합한 속도 피드백 계산(TBD-HW-011) |
| ESP32 모터 제어 | BTS7960 2개 PWM/DIR 제어, 좌·우 목표 속도 실행, 통신 watchdog, 오류 시 출력 차단 |
| ESP32 센서 제어 | MT6701 엔코더 2개(좌 구동·우 구동) 계수, 차체 IMU·DHT-11·HC-SR04 수집, 초음파 근거리 감지, 통신 watchdog |
| 모터 드라이버(BTS7960 ×2) | 모터 전력 구동, 과전류·과열 보호, enable/brake 입력 |
| 물리 E-Stop | 소프트웨어 상태와 무관하게 BTS7960 2개의 모터 전원 또는 driver enable을 하드웨어로 차단 |
...
- 두 ESP32 모두 PWM(MCPWM/LEDC), 필요한 3.3V GPIO 수, watchdog, USB 재연결 및 reset 동작을 실측한다. 모터 ESP32는 2채널 PWM/RPWM·LPWM/ENABLE, 센서 ESP32는 I2C 또는 ABZ 입력과 GPIO input capture를 확보해야 한다.
...
관제와 Nav2 구간에서는 표준 Twist의 선속도 `v`와 각속도 `ω`를 사용한다. Jetson의 차량 운동학 변환 계층(`vehicle_kinematics_node`)은 이를 ESP32 모터 제어용 **좌·우 목표 구동 속도**로 변환한다. 별도 조향 모터가 없으므로 회두는 전적으로 후륜 좌·우 속도 차이(차동 구동)로 만든다. 전방은 수동 캐스터 2개가 지지하며 차체 회두에 따라 자유롭게 방향을 튼다.

| 구간 | 최종 표현 |
|---|---|
| 관제·ROS 2 | `linearVelocityMps`, `angularVelocityRadps` |
| Jetson → ESP32 모터 제어 | `targetDriveLeftMmps`, `targetDriveRightMmps` |
| ESP32 센서 제어 → Jetson | 좌/우 구동 엔코더 tick·속도, 차체 IMU 원시 gyro·accel·측정 시각, DHT-11, HC-SR04 |
| ESP32 모터 제어 → Jetson | 적용 PWM(좌/우), fault |
| 차량 기하 | 실물 확인 후 트랙폭 `W`(좌·우 구동 바퀴 간격), 최대 각속도 `ω_max` |
...
Jetson은 물리 단위의 좌·우 구동 속도를 계산해 ESP32 모터 제어에 전달한다.
...
`W`(트랙폭, 좌·우 구동 바퀴 간격)는 실측값을 사용한다. 차동 구동이므로 `|v|`가 0이고 `ω`만 존재하는 명령은 좌·우 바퀴를 역방향으로 돌리는 **제자리 회전**으로 정상 실행한다(`v_left = −ω·W/2`, `v_right = +ω·W/2`). 각속도는 `ω_max`로 제한하고, 안전 변환 계층은 좌·우 속도가 물리 한계를 넘으면 비율을 유지한 채 함께 축소한다. Nav2는 최소 회전반경 제약이 없는 차동 구동 로봇으로 설정한다.
...
7. AUTO/MANUAL 명령 선택
8. 속도·가속도 제한
9. 좌·우 구동 속도 목표를 ESP32 모터 제어로 전송
```
...
→ vehicle_kinematics_node (차동 구동 변환)
...
| ESP32 모터 통신 watchdog | 정상 `DRIVE_COMMAND`가 300ms 이상 없음 | PWM=0(좌/우), 브레이크, driver disable 선택 |
...
| 작업 | 주기 |
|---|---:|
| 구동 엔코더 하드웨어 카운터 읽기(좌/우, MT6701) | 1kHz 또는 타이머 연속 계수 |
...
구동 모터는 좌·우 각각 엔코더 기반 속도 PID를 사용한다. 초기에는 P 또는 PI 제어부터 시작하고, 공중 무부하 값이 아니라 바닥에 놓은 저속 직진 시험으로 조정한다. 회두는 좌·우 목표 속도 차이(차동 구동)로만 만들며 별도 조향 제어 루프가 없다. 제자리 회전 시에는 좌·우가 반대 부호로 구동되므로 방향 전환 구간의 데드밴드·중립 처리를 좌·우 대칭으로 맞춘다.
...
| 2 | `DRIVE_ENCODER_FAULT` | 센서 | 좌/우 구동 엔코더(MT6701) 무응답·비정상 |
...
→ 퓨즈
→ 래칭형 NC E-Stop
→ BTS7960 2개(좌 구동·우 구동) 전원/contactor
→ RS540 ×2
...
- E-Stop 부품은 BTS7960 2개의 합산 최대 전류를 직접 감당하거나 적절한 contactor/relay를 제어해야 한다.
...
├─ sentinel_control/
│  ├─ command_mux_node
│  ├─ vehicle_kinematics_node   # v/ω → 좌·우 구동 속도(차동 구동)
│  ├─ safety_gate_node
│  └─ control_limits.yaml
└─ esp32_bridge/
   ├─ esp32_motor_bridge_node
...
| CTRL-07 | 좌/우 구동 엔코더 방향 | 전진 tick 부호(좌/우 개별)가 정의와 일치, 제자리 회전 시 좌·우 부호가 반대 |
...
| CTRL-14 | 각속도 한계 초과 명령 | 좌·우 속도가 비율을 유지한 채 `v_max`에서 포화되고 모터 과부하 없음 |
| CTRL-15 | 0m/s+각속도(제자리 회전) 명령 | 좌·우 바퀴가 반대 부호로 구동되어 안정적으로 제자리 회전, 캐스터 자유 선회 확인 |
...
| CTRL-18 | 초음파 임계거리 진입 | 센서 ESP32가 즉시 `protective_stop` 보고, Jetson 중계로 모터 ESP32가 정지, 왕복 지연 실측·기록(TBD-HW-011). **중계는 구현됐고(S15P11A301-237) 판정 로직만 단위 시험으로 고정된 상태다 — 왕복 지연은 미실측.** 임계는 **실측 0.15~0.18m**(S15P11A301-272) 이므로 이 시험은 접촉 직전 방어만 판정한다 |
...
> Sentinel UGV는 Jetson이 생성한 선속도·각속도를 차동 구동 기준의 좌·우 구동 속도로 변환해 유선 USB Serial로 모터 제어용 ESP32에 50Hz 전송한다. 별도 조향 모터가 없으며 회두와 제자리 회전은 후륜 좌·우 속도 차이로만 만들고, 전방은 수동 캐스터 2개가 지지한다. 모터 제어용 ESP32는 ESP-IDF 기반으로 BTS7960 2개(좌 구동·우 구동)를 구동하고 통신·내부 watchdog을 담당하며, 센서 제어용 ESP32는 별도 USB Serial로 MT6701 엔코더 2개·차체 IMU 1개·DHT-11·HC-SR04를 수집해 측정 시각·sequence와 함께 Jetson에 전달한다. Jetson은 `/imu/data_raw`를 발행하고 wheel odometry와 IMU yaw rate를 EKF로 융합한다. 두 ESP32 사이에는 직접 배선이 없으므로 속도 폐루프와 초음파 근거리 정지는 Jetson이 두 채널을 중계해 계산·실행하는 구조로 잠정 확정하며, 통신 지연이 문제가 되면 ESP32 간 직접 링크 추가를 TBD-HW-011로 검토한다. 유효 명령이 300ms 동안 없으면 모터 ESP32가 서버나 Jetson 프로세스와 무관하게 구동 모터를 정지하며, 물리 E-Stop은 모든 소프트웨어와 독립적으로 2개 드라이버의 구동 전원을 차단한다.
...
앞 단계가 틀린 상태에서 Nav2 파라미터를 먼저 만지면 원인을 구분할 수 없다. 모든 결과는 날짜·장소·차량 중량·타이어 상태·캐스터 상태와 함께 YAML과 측정표로 남긴다.
...
**URDF 가 이 트리와 일치한다** (2026-08-05 실측, S15P11A301-279). 종전에는 그렇지
않았다 — 짐벌이 v1.1-r4 에서 제외됐는데 `sentinel.urdf` 에만 링크 3개가 남아
라이다와 카메라가 거기 매달려 있었다.
...
> **`lidar_link`·`camera_link` 오프셋은 여전히 미실측이다.** 라이다 z=0.02 를 빼면
> 전부 0 인데 실제 차체에서 둘은 다른 위치에 있다. IMU 도 같은 상태다(TBD-HW-012).
> 이 정리는 값을 바꾸지 않았다 — 자를 대는 작업은 별개이며, 02장 6.4 가
> 「고정 마운트 상태로 실측해 TF 에 반영」으로 남겨 둔 항목이다.
...
## 35-3. 엔코더·바퀴·차동 구동 보정

### 방향·PPR 확인

1. 구동 바퀴를 바닥에서 띄운다.
2. 좌측 구동 모터를 낮은 PWM으로 전진시켜 좌측 엔코더 누적값이 정의된 양의 방향인지 확인한다. 우측도 동일하게 반복한다.
3. 좌·우를 반대 부호로 구동해 제자리 회전 방향(+ω 반시계)이 정의와 일치하는지 확인한다.
...
후륜에 기계식 차동장치가 없으므로 직진 시 좌·우 목표 속도가 동일해도 실제 바퀴 반경·타이어 마찰·캐스터 선회 저항 차이로 편향(drift)이 발생할 수 있다. 3m 직진 시험에서 좌·우 편차가 크면 좌·우 개별 `effective_meters_per_tick` 보정계수를 분리 적용하고, 필요하면 목표 속도에 좌·우 트림(bias)을 추가한다.

### v2.0: 캐스터 슬립이 오도메트리 yaw 를 갉아먹는다

직진 편향보다 **회전 오차가 더 크다.** 차동 구동에서 yaw 는 좌·우 바퀴 이동 거리 차로 계산하는데, 전방 수동 캐스터가 회전 중에 미끄러지면 바퀴는 돌았지만 차체는 그만큼 돌지 않는다. 그러면 **엔코더가 실제보다 큰 회전을 보고한다.** 누적되면 몇 분 뒤 방향이 수십 도 어긋난다.

이것이 `ekf_node`(엔코더 + IMU gyro 융합)가 **선택이 아니라 필수**인 이유다. 자이로는 슬립과 무관하게 실제 각속도를 측정하므로 이 오차를 크게 잡는다.

`slam_toolbox` 가 `map → odom` 을 보정하지만 그것은 지도 기준 위치만 맞추는 것이고, Nav2 의 local costmap 과 컨트롤러는 `odom` 프레임의 **국소 매끄러움**에 의존한다. 그 층에서 튀면 회전 중 경로 추종이 흔들린다. 컨트롤러를 Regulated Pure Pursuit 로 고른 것도 같은 이유다(04장 24.1).

측정 방법: 제자리 360° 회전을 손으로 시켜 엔코더 yaw 와 실제 회전량을 비교한다. EKF 를 끄고 켠 두 경우를 같은 기동으로 재고 개선을 수치로 남긴다. 이 값은 TBD-CAL-001 의 "최대 각속도·제자리 회전 대칭"과 함께 기록한다.
...
1. 좌·우 구동 바퀴 접지 중심 간격(트랙폭 `W`)을 실측한다. 차동 구동 운동학의 핵심 파라미터다.
2. 좌·우 동일 속도 명령으로 저속 원주행 대신 좌·우 속도 차이를 준 선회를 저속으로 수행하고, 명령 `ω`와 IMU·엔코더로 측정한 실제 yaw rate를 비교한다.
3. `v=0`, `ω`만 주는 제자리 회전을 좌·우(반시계·시계) 각 5회 수행해 명령 각속도와 실제 각속도의 매핑을 측정한다.
4. 명령 `ω`와 실제 yaw rate의 스케일 오차가 크면 트랙폭 `W`와 좌·우 `effective_meters_per_tick`를 먼저 재점검한다.

```text
ω_measured = (v_right - v_left) / W
W_effective = (v_right - v_left) / ω_measured   # 실측 yaw rate로 트랙폭 역산
```

좌·우 회전 대칭이 맞지 않으면 좌·우 구동 게인, 타이어 마찰, 캐스터 선회 저항을 먼저 점검한다. 차동 구동이므로 제자리 회전을 정상 기능으로 시험하며, Nav2에는 최소 회전반경 제약을 두지 않는다.

합격 기준:

- 좌·우 각각 3m 직진 5회 평균 거리 오차 ±5% 이내, 좌·우 편차 ±3%p 이내
- 일정 각속도 선회 종료 시 yaw 오차 ±10° 이내
- 제자리 회전 반시계·시계 각속도 대칭 오차 ±10% 이내
...
회두는 좌·우 목표 속도 차이로만 만드므로 별도 조향 튜닝이 없다. 대신 제자리 회전(`v=0`, `ω` 계단 입력)에서 좌·우 대칭성, 정정시간, yaw rate overshoot를 측정하고, 방향 전환 데드밴드가 좌·우 다르면 feedforward 표를 좌·우 별도로 보정한다.
...
- 제자리 회전 시 좌·우 대칭 오차와 yaw rate 응답
- 방향 전환(전진↔후진, 좌↔우 회전) 데드밴드
...
| 최대 각속도 | 0.6rad/s | 제자리 회전 안정성·좌우 구동 대칭·SLAM 품질 |
| 선가속도 | 0.25m/s² | 전류·기구 흔들림 |
| 각가속도 | 0.6rad/s² | 회전 overshoot |
| 사람 접근 속도 | 0.10m/s 이하 | 안전거리 시험 |
| 목표 사람 정지거리 | 1.5~2.0m | LiDAR·카메라 시야 |

정확한 footprint는 전방 캐스터·센서·범퍼의 최대 돌출 범위를 포함한다. 제자리 회전을 하므로 회전 중 어느 방향으로도 장애물에 닿지 않도록 회전 포락선을 고려하고, `robot_radius`로 단순 대체하지 말고 다각형 footprint를 사용한다.
...
| CAL-04 | 트랙폭·제자리 회전 | 실측 트랙폭 `W` 기록, 제자리 회전 반시계·시계 각속도 대칭 오차 ±10% 이내 |
...
> 캘리브레이션은 ESP32 좌·우 구동 엔코더·속도 제어부터 시작해 트랙폭·제자리 회전 각속도, 차체 IMU, 센서 TF, SLAM, Nav2, YOLO 순서로 진행한다. 바퀴 이동 스케일은 좌·우 각각 보정하고, 트랙폭 `W`와 명령 각속도 대비 실제 yaw rate 매핑을 실험값으로 보정한다. 별도 조향 모터가 없어 회두는 차동 구동으로만 만들고 최소 회전반경 제약은 두지 않는다. 짐벌은 사용하지 않으므로 카메라·LiDAR 외부 파라미터는 고정 마운트 기준으로 한 번만 확정하면 된다. 모든 보정값은 버전이 있는 YAML로 관리하고 각 임무에 펌웨어·모델·캘리브레이션 버전을 함께 저장한다.
