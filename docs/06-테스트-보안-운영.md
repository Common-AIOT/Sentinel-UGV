...
**명시적으로 수락한 위험**이다. 실제로 범위를 좁히고 있는 것은 위의 셋(쓰기 차단·토픽 6개·TLS)이며 **없는 것은 인증 하나**다.
...
| 젯슨 8765 무인증 읽기 | **읽기 자체는 완화 없음** — 관제 지도가 이 브릿지에 직접 붙어 상시 개방이다(2026-08-06 정정). 범위만 좁혔다: 쓰기 차단·토픽 6개·TLS. 카메라 영상은 제외 |
