...
- **자동 복귀(`RETURNING`)는 미구현**입니다. home pose 저장까지만 되어 있고 복귀 주행이 없어,
  임무는 `COMPLETED`로 끝나고 로봇은 종료 지점에 섭니다.
- **자동 종료도 미구현**입니다. 7분 타임아웃과 Frontier 소진 종료가 없어 운영자 STOP으로만
  끝납니다(배터리 기반 종료는 v2.0에서 폐기 — 전압·전류 센서 미장착).
- **사람의 map 좌표(`human_localizer`)는 미구현**입니다. encounter pose는 확정 시점의 로봇
  위치이며, 카메라 방위각·LiDAR 거리 결합은 접근 제어에만 씁니다. 카메라 hfov 실측이 남아 접근
  방향에 미세 오차가 있습니다(S15P11A301-371).
- 전·후방 초음파는 장착·발행까지 구현했으나 전방은 빈 공간 오측 때문에 발동을 껐고(커넥터 분리
  유지) 후방은 정지 판정에 넣지 않았습니다. 거리는 `/range/front`·`/range/rear`로 발행됩니다.
- 구동 속도·조향은 엔코더 폐루프 PID가 아니라 실측 매핑 기반 **개루프**입니다.
- 브라우저 bbox 오버레이, `detections` 적재, 로컬/원격 스트림 자동 전환, Continuous Aggregate·
  retention 정책은 미구현입니다.
- 프런트엔드의 `USE_MOCK`는 현재 `true`인 코드 상수입니다([RobotContext.tsx](frontend/features/robot/RobotContext.tsx#L34)).
  실제 관제 연동 배포 전 반드시 확인합니다.
- **애플리케이션 인증이 없습니다.** Spring API `/api/**`, MediaMTX 스트림 경로, Jetson
  8765 Foxglove 읽기 경로 모두 인증 없이 열립니다. Control Session은 조종권 중재이지 인증이
  아니므로 네트워크 허용 범위·CORS·TLS를 배포 경계로 취급합니다.
