...
| `ai/denoise` | `SilentAudioTrack`을 `NoAudioTrack`과 갈랐다. 무음은 업로드하지 않고 `[ALERT]`를 `scan.log`에 남긴다 |
...
| 보완된 벤치가 저장소에 반영되고 재실행 절차 문서화 | 🟡 부분. `tools/kspon_bench`(코퍼스 채점)와 `bench/pipeline_bench`는 있으나 **WER·슬롯 통합 채점기와 E2E 구간 계측이 없다** |
