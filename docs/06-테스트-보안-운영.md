...
## 16.2 하드웨어 테스트 체크리스트
차량을 바닥에서 띄운 상태에서 시작한다(38-6 환경 E1). STM32 부팅 시 PWM 0·enable LOW, USB 분리 후 300ms 이내 정지, E-Stop 시 모터 전원 차단, 프로그램 종료 시 정지는 38장 CTRL 시험군과 G0~G1 Gate에서 검증한다. 시험 ID가 없는 물리 점검 항목은 다음과 같다.
...
배포 완료는 프로세스가 실행되었다는 사실이 아니라 `STM32 handshake → 센서 health → 로컬 영상 → 가짜 임무 명령 → STOP` smoke test 통과로 판정한다.
