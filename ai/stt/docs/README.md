...
| **관제 ACK 수신** | ⛔ **존재하지 않음** — 로봇 다수 투입으로 관제가 개별 ACK를 내리지 않는다(2026-08-01 확정). `report_lifecycle.py` 삭제, **182 재평가 대상** |
...
>
> 아래 구분 중 **살아 있는 것은 "탐사 재개 승인" 하나**다 — E-Stop·중단 상태에서
> "다시 탐색을 시작합니다"를 말하면 거짓이 되므로 그때는 재생하지 않는다.
> 판단은 `ros_node`가 임무 상태로 하고, 게이트는 `requires_exploration_resume`이다.
> S15P11A301-182는 이 결정을 반영해 재평가 대상이다.
